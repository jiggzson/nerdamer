...
var nerdamer = (function(imports) { 
    ...
    var version = '0.7.13',
        ...
        Math2 = {
            ...
            ifactor_new: function(n, maxn) { 
                n = new bigInt(n); 
                maxn = maxn || 10000; //safety
                ...
                while(true) { 
                    ...
                    var remainder_is_zero = remainder.equals(0);
                    ...
                    factors[i]++;
                    //If the number is a prime then we're done since i will equal n and remainder will equal 0
                    if(remainder_is_zero && n.equals(i))
                        return factors;
    ...
    bigInt: (function(){
        return function(undefined){"use strict";var BASE=1e7,LOG_BASE=7,MAX_INT=9007199254740992,MAX_INT_ARR=smallToArray(MAX_INT),LOG_MAX_INT=Math.log(MAX_INT);function Integer(v,radix){if(typeof v==="undefined")return Integer[0];if(typeof radix!=="undefined")return+radix===10?parseValue(v):parseBase(v,radix);return parseValue(v)}function BigInteger(value,sign){this.value=value;this.sign=sign;this.isSmall=false}BigInteger.prototype=Object.create(Integer.prototype);function SmallInteger(value){this.value=value;this.sign=value<0;this.isSmall=true}SmallInteger.prototype=Object.create(Integer.prototype);function isPrecise(n){return-MAX_INT<n&&n<MAX_INT}function smallToArray(n){if(n<1e7)return[n];if(n<1e14)return[n%1e7,Math.floor(n/1e7)];return[n%1e7,Math.floor(n/1e7)%1e7,Math.floor(n/1e14)]}function arrayToSmall(arr){trim(arr);var length=arr.length;if(length<4&&compareAbs(arr,MAX_INT_ARR)<0){switch(length){case 0:return 0;case 1:return arr[0];case 2:return arr[0]+arr[1]*BASE;default:return arr[0]+(arr[1]+arr[2]*BASE)*BASE}}return arr}function trim(v){var i=v.length;while(v[--i]===0);v.length=i+1}function createArray(length){var x=new Array(length);var i=-1;while(++i<length){x[i]=0}return x}function truncate(n){if(n>0)return Math.floor(n);return Math.ceil(n)}function add(a,b){var l_a=a.length,l_b=b.length,r=new Array(l_a),carry=0,base=BASE,sum,i;for(i=0;i<l_b;i++){sum=a[i]+b[i]+carry;carry=sum>=base?1:0;r[i]=sum-carry*base}while(i<l_a){sum=a[i]+carry;carry=sum===base?1:0;r[i++]=sum-carry*base}if(carry>0)r.push(carry);return r}function addAny(a,b){if(a.length>=b.length)return add(a,b);return add(b,a)}function addSmall(a,carry){var l=a.length,r=new Array(l),base=BASE,sum,i;for(i=0;i<l;i++){sum=a[i]-base+carry;carry=Math.floor(sum/base);r[i]=sum-carry*base;carry+=1}while(carry>0){r[i++]=carry%base;carry=Math.floor(carry/base)}return r}BigInteger.prototype.add=function(v){var n=parseValue(v);if(this.sign!==n.sign){return this.subtract(n.negate())}var a=this.value,b=n.value;if(n.isSmall){return new BigInteger(addSmall(a,Math.abs(b)),this.sign)}return new BigInteger(addAny(a,b),this.sign)};BigInteger.prototype.plus=BigInteger.prototype.add;SmallInteger.prototype.add=function(v){var n=parseValue(v);var a=this.value;if(a<0!==n.sign){return this.subtract(n.negate())}var b=n.value;if(n.isSmall){if(isPrecise(a+b))return new SmallInteger(a+b);b=smallToArray(Math.abs(b))}return new BigInteger(addSmall(b,Math.abs(a)),a<0)};SmallInteger.prototype.plus=SmallInteger.prototype.add;function subtract(a,b){var a_l=a.length,b_l=b.length,r=new Array(a_l),borrow=0,base=BASE,i,difference;for(i=0;i<b_l;i++){difference=a[i]-borrow-b[i];if(difference<0){difference+=base;borrow=1}else borrow=0;r[i]=difference}for(i=b_l;i<a_l;i++){difference=a[i]-borrow;if(difference<0)difference+=base;else{r[i++]=difference;break}r[i]=difference}for(;i<a_l;i++){r[i]=a[i]}trim(r);return r}function subtractAny(a,b,sign){var value;if(compareAbs(a,b)>=0){value=subtract(a,b)}else{value=subtract(b,a);sign=!sign}value=arrayToSmall(value);if(typeof value==="number"){if(sign)value=-value;return new SmallInteger(value)}return new BigInteger(value,sign)}function subtractSmall(a,b,sign){var l=a.length,r=new Array(l),carry=-b,base=BASE,i,difference;for(i=0;i<l;i++){difference=a[i]+carry;carry=Math.floor(difference/base);difference%=base;r[i]=difference<0?difference+base:difference}r=arrayToSmall(r);if(typeof r==="number"){if(sign)r=-r;return new SmallInteger(r)}return new BigInteger(r,sign)}BigInteger.prototype.subtract=function(v){var n=parseValue(v);if(this.sign!==n.sign){return this.add(n.negate())}var a=this.value,b=n.value;if(n.isSmall)return subtractSmall(a,Math.abs(b),this.sign);return subtractAny(a,b,this.sign)};BigInteger.prototype.minus=BigInteger.prototype.subtract;SmallInteger.prototype.subtract=function(v){var n=parseValue(v);var a=this.value;if(a<0!==n.sign){return this.add(n.negate())}var b=n.value;if(n.isSmall){return new SmallInteger(a-b)}return subtractSmall(b,Math.abs(a),a>=0)};SmallInteger.prototype.minus=SmallInteger.prototype.subtract;BigInteger.prototype.negate=function(){return new BigInteger(this.value,!this.sign)};SmallInteger.prototype.negate=function(){var sign=this.sign;var small=new SmallInteger(-this.value);small.sign=!sign;return small};BigInteger.prototype.abs=function(){return new BigInteger(this.value,false)};SmallInteger.prototype.abs=function(){return new SmallInteger(Math.abs(this.value))};function multiplyLong(a,b){var a_l=a.length,b_l=b.length,l=a_l+b_l,r=createArray(l),base=BASE,product,carry,i,a_i,b_j;for(i=0;i<a_l;++i){a_i=a[i];for(var j=0;j<b_l;++j){b_j=b[j];product=a_i*b_j+r[i+j];carry=Math.floor(product/base);r[i+j]=product-carry*base;r[i+j+1]+=carry}}trim(r);return r}function multiplySmall(a,b){var l=a.length,r=new Array(l),base=BASE,carry=0,product,i;for(i=0;i<l;i++){product=a[i]*b+carry;carry=Math.floor(product/base);r[i]=product-carry*base}while(carry>0){r[i++]=carry%base;carry=Math.floor(carry/base)}return r}function shiftLeft(x,n){var r=[];while(n-- >0)r.push(0);return r.concat(x)}function multiplyKaratsuba(x,y){var n=Math.max(x.length,y.length);if(n<=30)return multiplyLong(x,y);n=Math.ceil(n/2);var b=x.slice(n),a=x.slice(0,n),d=y.slice(n),c=y.slice(0,n);var ac=multiplyKaratsuba(a,c),bd=multiplyKaratsuba(b,d),abcd=multiplyKaratsuba(addAny(a,b),addAny(c,d));var product=addAny(addAny(ac,shiftLeft(subtract(subtract(abcd,ac),bd),n)),shiftLeft(bd,2*n));trim(product);return product}function useKaratsuba(l1,l2){return-.012*l1-.012*l2+15e-6*l1*l2>0}BigInteger.prototype.multiply=function(v){var n=parseValue(v),a=this.value,b=n.value,sign=this.sign!==n.sign,abs;if(n.isSmall){if(b===0)return Integer[0];if(b===1)return this;if(b===-1)return this.negate();abs=Math.abs(b);if(abs<BASE){return new BigInteger(multiplySmall(a,abs),sign)}b=smallToArray(abs)}if(useKaratsuba(a.length,b.length))return new BigInteger(multiplyKaratsuba(a,b),sign);return new BigInteger(multiplyLong(a,b),sign)};BigInteger.prototype.times=BigInteger.prototype.multiply;function multiplySmallAndArray(a,b,sign){if(a<BASE){return new BigInteger(multiplySmall(b,a),sign)}return new BigInteger(multiplyLong(b,smallToArray(a)),sign)}SmallInteger.prototype._multiplyBySmall=function(a){if(isPrecise(a.value*this.value)){return new SmallInteger(a.value*this.value)}return multiplySmallAndArray(Math.abs(a.value),smallToArray(Math.abs(this.value)),this.sign!==a.sign)};BigInteger.prototype._multiplyBySmall=function(a){if(a.value===0)return Integer[0];if(a.value===1)return this;if(a.value===-1)return this.negate();return multiplySmallAndArray(Math.abs(a.value),this.value,this.sign!==a.sign)};SmallInteger.prototype.multiply=function(v){return parseValue(v)._multiplyBySmall(this)};SmallInteger.prototype.times=SmallInteger.prototype.multiply;function square(a){var l=a.length,r=createArray(l+l),base=BASE,product,carry,i,a_i,a_j;for(i=0;i<l;i++){a_i=a[i];for(var j=0;j<l;j++){a_j=a[j];product=a_i*a_j+r[i+j];carry=Math.floor(product/base);r[i+j]=product-carry*base;r[i+j+1]+=carry}}trim(r);return r}BigInteger.prototype.square=function(){return new BigInteger(square(this.value),false)};SmallInteger.prototype.square=function(){var value=this.value*this.value;if(isPrecise(value))return new SmallInteger(value);return new BigInteger(square(smallToArray(Math.abs(this.value))),false)};function divMod1(a,b){var a_l=a.length,b_l=b.length,base=BASE,result=createArray(b.length),divisorMostSignificantDigit=b[b_l-1],lambda=Math.ceil(base/(2*divisorMostSignificantDigit)),remainder=multiplySmall(a,lambda),divisor=multiplySmall(b,lambda),quotientDigit,shift,carry,borrow,i,l,q;if(remainder.length<=a_l)remainder.push(0);divisor.push(0);divisorMostSignificantDigit=divisor[b_l-1];for(shift=a_l-b_l;shift>=0;shift--){quotientDigit=base-1;if(remainder[shift+b_l]!==divisorMostSignificantDigit){quotientDigit=Math.floor((remainder[shift+b_l]*base+remainder[shift+b_l-1])/divisorMostSignificantDigit)}carry=0;borrow=0;l=divisor.length;for(i=0;i<l;i++){carry+=quotientDigit*divisor[i];q=Math.floor(carry/base);borrow+=remainder[shift+i]-(carry-q*base);carry=q;if(borrow<0){remainder[shift+i]=borrow+base;borrow=-1}else{remainder[shift+i]=borrow;borrow=0}}while(borrow!==0){quotientDigit-=1;carry=0;for(i=0;i<l;i++){carry+=remainder[shift+i]-base+divisor[i];if(carry<0){remainder[shift+i]=carry+base;carry=0}else{remainder[shift+i]=carry;carry=1}}borrow+=carry}result[shift]=quotientDigit}remainder=divModSmall(remainder,lambda)[0];return[arrayToSmall(result),arrayToSmall(remainder)]}function divMod2(a,b){var a_l=a.length,b_l=b.length,result=[],part=[],base=BASE,guess,xlen,highx,highy,check;while(a_l){part.unshift(a[--a_l]);trim(part);if(compareAbs(part,b)<0){result.push(0);continue}xlen=part.length;highx=part[xlen-1]*base+part[xlen-2];highy=b[b_l-1]*base+b[b_l-2];if(xlen>b_l){highx=(highx+1)*base}guess=Math.ceil(highx/highy);do{check=multiplySmall(b,guess);if(compareAbs(check,part)<=0)break;guess--}while(guess);result.push(guess);part=subtract(part,check)}result.reverse();return[arrayToSmall(result),arrayToSmall(part)]}function divModSmall(value,lambda){var length=value.length,quotient=createArray(length),base=BASE,i,q,remainder,divisor;remainder=0;for(i=length-1;i>=0;--i){divisor=remainder*base+value[i];q=truncate(divisor/lambda);remainder=divisor-q*lambda;quotient[i]=q|0}return[quotient,remainder|0]}function divModAny(self,v){var value,n=parseValue(v);var a=self.value,b=n.value;var quotient;if(b===0)throw new Error("Cannot divide by zero");if(self.isSmall){if(n.isSmall){return[new SmallInteger(truncate(a/b)),new SmallInteger(a%b)]}return[Integer[0],self]}if(n.isSmall){if(b===1)return[self,Integer[0]];if(b==-1)return[self.negate(),Integer[0]];var abs=Math.abs(b);if(abs<BASE){value=divModSmall(a,abs);quotient=arrayToSmall(value[0]);var remainder=value[1];if(self.sign)remainder=-remainder;if(typeof quotient==="number"){if(self.sign!==n.sign)quotient=-quotient;return[new SmallInteger(quotient),new SmallInteger(remainder)]}return[new BigInteger(quotient,self.sign!==n.sign),new SmallInteger(remainder)]}b=smallToArray(abs)}var comparison=compareAbs(a,b);if(comparison===-1)return[Integer[0],self];if(comparison===0)return[Integer[self.sign===n.sign?1:-1],Integer[0]];if(a.length+b.length<=200)value=divMod1(a,b);else value=divMod2(a,b);quotient=value[0];var qSign=self.sign!==n.sign,mod=value[1],mSign=self.sign;if(typeof quotient==="number"){if(qSign)quotient=-quotient;quotient=new SmallInteger(quotient)}else quotient=new BigInteger(quotient,qSign);if(typeof mod==="number"){if(mSign)mod=-mod;mod=new SmallInteger(mod)}else mod=new BigInteger(mod,mSign);return[quotient,mod]}BigInteger.prototype.divmod=function(v){var result=divModAny(this,v);return{quotient:result[0],remainder:result[1]}};SmallInteger.prototype.divmod=BigInteger.prototype.divmod;BigInteger.prototype.divide=function(v){return divModAny(this,v)[0]};SmallInteger.prototype.over=SmallInteger.prototype.divide=BigInteger.prototype.over=BigInteger.prototype.divide;BigInteger.prototype.mod=function(v){return divModAny(this,v)[1]};SmallInteger.prototype.remainder=SmallInteger.prototype.mod=BigInteger.prototype.remainder=BigInteger.prototype.mod;BigInteger.prototype.pow=function(v){var n=parseValue(v),a=this.value,b=n.value,value,x,y;if(b===0)return Integer[1];if(a===0)return Integer[0];if(a===1)return Integer[1];if(a===-1)return n.isEven()?Integer[1]:Integer[-1];if(n.sign){return Integer[0]}if(!n.isSmall)throw new Error("The exponent "+n.toString()+" is too large.");if(this.isSmall){if(isPrecise(value=Math.pow(a,b)))return new SmallInteger(truncate(value))}x=this;y=Integer[1];while(true){if(b&1===1){y=y.times(x);--b}if(b===0)break;b/=2;x=x.square()}return y};SmallInteger.prototype.pow=BigInteger.prototype.pow;BigInteger.prototype.modPow=function(exp,mod){exp=parseValue(exp);mod=parseValue(mod);if(mod.isZero())throw new Error("Cannot take modPow with modulus 0");var r=Integer[1],base=this.mod(mod);while(exp.isPositive()){if(base.isZero())return Integer[0];if(exp.isOdd())r=r.multiply(base).mod(mod);exp=exp.divide(2);base=base.square().mod(mod)}return r};SmallInteger.prototype.modPow=BigInteger.prototype.modPow;function compareAbs(a,b){if(a.length!==b.length){return a.length>b.length?1:-1}for(var i=a.length-1;i>=0;i--){if(a[i]!==b[i])return a[i]>b[i]?1:-1}return 0}BigInteger.prototype.compareAbs=function(v){var n=parseValue(v),a=this.value,b=n.value;if(n.isSmall)return 1;return compareAbs(a,b)};SmallInteger.prototype.compareAbs=function(v){var n=parseValue(v),a=Math.abs(this.value),b=n.value;if(n.isSmall){b=Math.abs(b);return a===b?0:a>b?1:-1}return-1};BigInteger.prototype.compare=function(v){if(v===Infinity){return-1}if(v===-Infinity){return 1}var n=parseValue(v),a=this.value,b=n.value;if(this.sign!==n.sign){return n.sign?1:-1}if(n.isSmall){return this.sign?-1:1}return compareAbs(a,b)*(this.sign?-1:1)};BigInteger.prototype.compareTo=BigInteger.prototype.compare;SmallInteger.prototype.compare=function(v){if(v===Infinity){return-1}if(v===-Infinity){return 1}var n=parseValue(v),a=this.value,b=n.value;if(n.isSmall){return a==b?0:a>b?1:-1}if(a<0!==n.sign){return a<0?-1:1}return a<0?1:-1};SmallInteger.prototype.compareTo=SmallInteger.prototype.compare;BigInteger.prototype.equals=function(v){return this.compare(v)===0};SmallInteger.prototype.eq=SmallInteger.prototype.equals=BigInteger.prototype.eq=BigInteger.prototype.equals;BigInteger.prototype.notEquals=function(v){return this.compare(v)!==0};SmallInteger.prototype.neq=SmallInteger.prototype.notEquals=BigInteger.prototype.neq=BigInteger.prototype.notEquals;BigInteger.prototype.greater=function(v){return this.compare(v)>0};SmallInteger.prototype.gt=SmallInteger.prototype.greater=BigInteger.prototype.gt=BigInteger.prototype.greater;BigInteger.prototype.lesser=function(v){return this.compare(v)<0};SmallInteger.prototype.lt=SmallInteger.prototype.lesser=BigInteger.prototype.lt=BigInteger.prototype.lesser;BigInteger.prototype.greaterOrEquals=function(v){return this.compare(v)>=0};SmallInteger.prototype.geq=SmallInteger.prototype.greaterOrEquals=BigInteger.prototype.geq=BigInteger.prototype.greaterOrEquals;BigInteger.prototype.lesserOrEquals=function(v){return this.compare(v)<=0};SmallInteger.prototype.leq=SmallInteger.prototype.lesserOrEquals=BigInteger.prototype.leq=BigInteger.prototype.lesserOrEquals;BigInteger.prototype.isEven=function(){return(this.value[0]&1)===0};SmallInteger.prototype.isEven=function(){return(this.value&1)===0};BigInteger.prototype.isOdd=function(){return(this.value[0]&1)===1};SmallInteger.prototype.isOdd=function(){return(this.value&1)===1};BigInteger.prototype.isPositive=function(){return!this.sign};SmallInteger.prototype.isPositive=function(){return this.value>0};BigInteger.prototype.isNegative=function(){return this.sign};SmallInteger.prototype.isNegative=function(){return this.value<0};BigInteger.prototype.isUnit=function(){return false};SmallInteger.prototype.isUnit=function(){return Math.abs(this.value)===1};BigInteger.prototype.isZero=function(){return false};SmallInteger.prototype.isZero=function(){return this.value===0};BigInteger.prototype.isDivisibleBy=function(v){var n=parseValue(v);var value=n.value;if(value===0)return false;if(value===1)return true;if(value===2)return this.isEven();return this.mod(n).equals(Integer[0])};SmallInteger.prototype.isDivisibleBy=BigInteger.prototype.isDivisibleBy;function isBasicPrime(v){var n=v.abs();if(n.isUnit())return false;if(n.equals(2)||n.equals(3)||n.equals(5))return true;if(n.isEven()||n.isDivisibleBy(3)||n.isDivisibleBy(5))return false;if(n.lesser(25))return true}BigInteger.prototype.isPrime=function(){var isPrime=isBasicPrime(this);if(isPrime!==undefined)return isPrime;var n=this.abs(),nPrev=n.prev();var a=[2,3,5,7,11,13,17,19],b=nPrev,d,t,i,x;while(b.isEven())b=b.divide(2);for(i=0;i<a.length;i++){x=bigInt(a[i]).modPow(b,n);if(x.equals(Integer[1])||x.equals(nPrev))continue;for(t=true,d=b;t&&d.lesser(nPrev);d=d.multiply(2)){x=x.square().mod(n);if(x.equals(nPrev))t=false}if(t)return false}return true};SmallInteger.prototype.isPrime=BigInteger.prototype.isPrime;BigInteger.prototype.isProbablePrime=function(iterations){var isPrime=isBasicPrime(this);if(isPrime!==undefined)return isPrime;var n=this.abs();var t=iterations===undefined?5:iterations;for(var i=0;i<t;i++){var a=bigInt.randBetween(2,n.minus(2));if(!a.modPow(n.prev(),n).isUnit())return false}return true};SmallInteger.prototype.isProbablePrime=BigInteger.prototype.isProbablePrime;BigInteger.prototype.modInv=function(n){var t=bigInt.zero,newT=bigInt.one,r=parseValue(n),newR=this.abs(),q,lastT,lastR;while(!newR.equals(bigInt.zero)){q=r.divide(newR);lastT=t;lastR=r;t=newT;r=newR;newT=lastT.subtract(q.multiply(newT));newR=lastR.subtract(q.multiply(newR))}if(!r.equals(1))throw new Error(this.toString()+" and "+n.toString()+" are not co-prime");if(t.compare(0)===-1){t=t.add(n)}if(this.isNegative()){return t.negate()}return t};SmallInteger.prototype.modInv=BigInteger.prototype.modInv;BigInteger.prototype.next=function(){var value=this.value;if(this.sign){return subtractSmall(value,1,this.sign)}return new BigInteger(addSmall(value,1),this.sign)};SmallInteger.prototype.next=function(){var value=this.value;if(value+1<MAX_INT)return new SmallInteger(value+1);return new BigInteger(MAX_INT_ARR,false)};BigInteger.prototype.prev=function(){var value=this.value;if(this.sign){return new BigInteger(addSmall(value,1),true)}return subtractSmall(value,1,this.sign)};SmallInteger.prototype.prev=function(){var value=this.value;if(value-1>-MAX_INT)return new SmallInteger(value-1);return new BigInteger(MAX_INT_ARR,true)};var powersOfTwo=[1];while(powersOfTwo[powersOfTwo.length-1]<=BASE)powersOfTwo.push(2*powersOfTwo[powersOfTwo.length-1]);var powers2Length=powersOfTwo.length,highestPower2=powersOfTwo[powers2Length-1];function shift_isSmall(n){return(typeof n==="number"||typeof n==="string")&&+Math.abs(n)<=BASE||n instanceof BigInteger&&n.value.length<=1}BigInteger.prototype.shiftLeft=function(n){if(!shift_isSmall(n)){throw new Error(String(n)+" is too large for shifting.")}n=+n;if(n<0)return this.shiftRight(-n);var result=this;while(n>=powers2Length){result=result.multiply(highestPower2);n-=powers2Length-1}return result.multiply(powersOfTwo[n])};SmallInteger.prototype.shiftLeft=BigInteger.prototype.shiftLeft;BigInteger.prototype.shiftRight=function(n){var remQuo;if(!shift_isSmall(n)){throw new Error(String(n)+" is too large for shifting.")}n=+n;if(n<0)return this.shiftLeft(-n);var result=this;while(n>=powers2Length){if(result.isZero())return result;remQuo=divModAny(result,highestPower2);result=remQuo[1].isNegative()?remQuo[0].prev():remQuo[0];n-=powers2Length-1}remQuo=divModAny(result,powersOfTwo[n]);return remQuo[1].isNegative()?remQuo[0].prev():remQuo[0]};SmallInteger.prototype.shiftRight=BigInteger.prototype.shiftRight;function bitwise(x,y,fn){y=parseValue(y);var xSign=x.isNegative(),ySign=y.isNegative();var xRem=xSign?x.not():x,yRem=ySign?y.not():y;var xBits=[],yBits=[];var xStop=false,yStop=false;while(!xStop||!yStop){if(xRem.isZero()){xStop=true;xBits.push(xSign?1:0)}else if(xSign)xBits.push(xRem.isEven()?1:0);else xBits.push(xRem.isEven()?0:1);if(yRem.isZero()){yStop=true;yBits.push(ySign?1:0)}else if(ySign)yBits.push(yRem.isEven()?1:0);else yBits.push(yRem.isEven()?0:1);xRem=xRem.over(2);yRem=yRem.over(2)}var result=[];for(var i=0;i<xBits.length;i++)result.push(fn(xBits[i],yBits[i]));var sum=bigInt(result.pop()).negate().times(bigInt(2).pow(result.length));while(result.length){sum=sum.add(bigInt(result.pop()).times(bigInt(2).pow(result.length)))}return sum}BigInteger.prototype.not=function(){return this.negate().prev()};SmallInteger.prototype.not=BigInteger.prototype.not;BigInteger.prototype.and=function(n){return bitwise(this,n,function(a,b){return a&b})};SmallInteger.prototype.and=BigInteger.prototype.and;BigInteger.prototype.or=function(n){return bitwise(this,n,function(a,b){return a|b})};SmallInteger.prototype.or=BigInteger.prototype.or;BigInteger.prototype.xor=function(n){return bitwise(this,n,function(a,b){return a^b})};SmallInteger.prototype.xor=BigInteger.prototype.xor;var LOBMASK_I=1<<30,LOBMASK_BI=(BASE&-BASE)*(BASE&-BASE)|LOBMASK_I;function roughLOB(n){var v=n.value,x=typeof v==="number"?v|LOBMASK_I:v[0]+v[1]*BASE|LOBMASK_BI;return x&-x}function max(a,b){a=parseValue(a);b=parseValue(b);return a.greater(b)?a:b}function min(a,b){a=parseValue(a);b=parseValue(b);return a.lesser(b)?a:b}function gcd(a,b){a=parseValue(a).abs();b=parseValue(b).abs();if(a.equals(b))return a;if(a.isZero())return b;if(b.isZero())return a;var c=Integer[1],d,t;while(a.isEven()&&b.isEven()){d=Math.min(roughLOB(a),roughLOB(b));a=a.divide(d);b=b.divide(d);c=c.multiply(d)}while(a.isEven()){a=a.divide(roughLOB(a))}do{while(b.isEven()){b=b.divide(roughLOB(b))}if(a.greater(b)){t=b;b=a;a=t}b=b.subtract(a)}while(!b.isZero());return c.isUnit()?a:a.multiply(c)}function lcm(a,b){a=parseValue(a).abs();b=parseValue(b).abs();return a.divide(gcd(a,b)).multiply(b)}function randBetween(a,b){a=parseValue(a);b=parseValue(b);var low=min(a,b),high=max(a,b);var range=high.subtract(low);if(range.isSmall)return low.add(Math.round(Math.random()*range));var length=range.value.length-1;var result=[],restricted=true;for(var i=length;i>=0;i--){var top=restricted?range.value[i]:BASE;var digit=truncate(Math.random()*top);result.unshift(digit);if(digit<top)restricted=false}result=arrayToSmall(result);return low.add(typeof result==="number"?new SmallInteger(result):new BigInteger(result,false))}var parseBase=function(text,base){var length=text.length;var i;var absBase=Math.abs(base);for(var i=0;i<length;i++){var c=text[i].toLowerCase();if(c==="-")continue;if(/[a-z0-9]/.test(c)){if(/[0-9]/.test(c)&&+c>=absBase){if(c==="1"&&absBase===1)continue;throw new Error(c+" is not a valid digit in base "+base+".")}else if(c.charCodeAt(0)-87>=absBase){throw new Error(c+" is not a valid digit in base "+base+".")}}}if(2<=base&&base<=36){if(length<=LOG_MAX_INT/Math.log(base)){var result=parseInt(text,base);if(isNaN(result)){throw new Error(c+" is not a valid digit in base "+base+".")}return new SmallInteger(parseInt(text,base))}}base=parseValue(base);var digits=[];var isNegative=text[0]==="-";for(i=isNegative?1:0;i<text.length;i++){var c=text[i].toLowerCase(),charCode=c.charCodeAt(0);if(48<=charCode&&charCode<=57)digits.push(parseValue(c));else if(97<=charCode&&charCode<=122)digits.push(parseValue(c.charCodeAt(0)-87));else if(c==="<"){var start=i;do{i++}while(text[i]!==">");digits.push(parseValue(text.slice(start+1,i)))}else throw new Error(c+" is not a valid character")}return parseBaseFromArray(digits,base,isNegative)};function parseBaseFromArray(digits,base,isNegative){var val=Integer[0],pow=Integer[1],i;for(i=digits.length-1;i>=0;i--){val=val.add(digits[i].times(pow));pow=pow.times(base)}return isNegative?val.negate():val}function stringify(digit){var v=digit.value;if(typeof v==="number")v=[v];if(v.length===1&&v[0]<=35){return"0123456789abcdefghijklmnopqrstuvwxyz".charAt(v[0])}return"<"+v+">"}function toBase(n,base){base=bigInt(base);if(base.isZero()){if(n.isZero())return"0";throw new Error("Cannot convert nonzero numbers to base 0.")}if(base.equals(-1)){if(n.isZero())return"0";if(n.isNegative())return new Array(1-n).join("10");return"1"+new Array(+n).join("01")}var minusSign="";if(n.isNegative()&&base.isPositive()){minusSign="-";n=n.abs()}if(base.equals(1)){if(n.isZero())return"0";return minusSign+new Array(+n+1).join(1)}var out=[];var left=n,divmod;while(left.isNegative()||left.compareAbs(base)>=0){divmod=left.divmod(base);left=divmod.quotient;var digit=divmod.remainder;if(digit.isNegative()){digit=base.minus(digit).abs();left=left.next()}out.push(stringify(digit))}out.push(stringify(left));return minusSign+out.reverse().join("")}BigInteger.prototype.toString=function(radix){if(radix===undefined)radix=10;if(radix!==10)return toBase(this,radix);var v=this.value,l=v.length,str=String(v[--l]),zeros="0000000",digit;while(--l>=0){digit=String(v[l]);str+=zeros.slice(digit.length)+digit}var sign=this.sign?"-":"";return sign+str};SmallInteger.prototype.toString=function(radix){if(radix===undefined)radix=10;if(radix!=10)return toBase(this,radix);return String(this.value)};BigInteger.prototype.toJSON=SmallInteger.prototype.toJSON=function(){return this.toString()};BigInteger.prototype.valueOf=function(){return+this.toString()};BigInteger.prototype.toJSNumber=BigInteger.prototype.valueOf;SmallInteger.prototype.valueOf=function(){return this.value};SmallInteger.prototype.toJSNumber=SmallInteger.prototype.valueOf;function parseStringValue(v){if(isPrecise(+v)){var x=+v;if(x===truncate(x))return new SmallInteger(x);throw"Invalid integer: "+v}var sign=v[0]==="-";if(sign)v=v.slice(1);var split=v.split(/e/i);if(split.length>2)throw new Error("Invalid integer: "+split.join("e"));if(split.length===2){var exp=split[1];if(exp[0]==="+")exp=exp.slice(1);exp=+exp;if(exp!==truncate(exp)||!isPrecise(exp))throw new Error("Invalid integer: "+exp+" is not a valid exponent.");var text=split[0];var decimalPlace=text.indexOf(".");if(decimalPlace>=0){exp-=text.length-decimalPlace-1;text=text.slice(0,decimalPlace)+text.slice(decimalPlace+1)}if(exp<0)throw new Error("Cannot include negative exponent part for integers");text+=new Array(exp+1).join("0");v=text}var isValid=/^([0-9][0-9]*)$/.test(v);if(!isValid)throw new Error("Invalid integer: "+v);var r=[],max=v.length,l=LOG_BASE,min=max-l;while(max>0){r.push(+v.slice(min,max));min-=l;if(min<0)min=0;max-=l}trim(r);return new BigInteger(r,sign)}function parseNumberValue(v){if(isPrecise(v)){if(v!==truncate(v))throw new Error(v+" is not an integer.");return new SmallInteger(v)}return parseStringValue(v.toString())}function parseValue(v){if(typeof v==="number"){return parseNumberValue(v)}if(typeof v==="string"){return parseStringValue(v)}return v}for(var i=0;i<1e3;i++){Integer[i]=new SmallInteger(i);if(i>0)Integer[-i]=new SmallInteger(-i)}Integer.one=Integer[1];Integer.zero=Integer[0];Integer.minusOne=Integer[-1];Integer.max=max;Integer.min=min;Integer.gcd=gcd;Integer.lcm=lcm;Integer.isInstance=function(x){return x instanceof BigInteger||x instanceof SmallInteger};Integer.randBetween=randBetween;Integer.fromArray=function(digits,base,isNegative){return parseBaseFromArray(digits.map(parseValue),parseValue(base||10),isNegative)};return Integer}();if(typeof module!=="undefined"&&module.hasOwnProperty("exports")){module.exports=bigInt}if(typeof define==="function"&&define.amd){define("big-integer",[],function(){return bigInt})}
    })(),
//    /* bignumber.js v4.0.4 https://github.com/MikeMcl/bignumber.js/LICENCE */
   bigDecimal:(function(){
        /*! bignumber.js v4.0.4 https://github.com/MikeMcl/bignumber.js/LICENCE */
        //return function(n){"use strict";function e(n){var e,i,r,t=n.length-1,s="",o=n[0];if(t>0){for(s+=o,e=1;e<t;e++)r=n[e]+"",(i=Pn-r.length)&&(s+=d(i)),s+=r;o=n[e],(i=Pn-(r=o+"").length)&&(s+=d(i))}else if(0===o)return"0";for(;o%10==0;)o/=10;return s+o}function i(n,e,i){if(n!==~~n||n<e||n>i)throw Error(En+n)}function r(n,e,i,r){var t,s,o,u;for(s=n[0];s>=10;s/=10)--e;return--e<0?(e+=Pn,t=0):(t=Math.ceil((e+1)/Pn),e%=Pn),s=qn(10,Pn-e),u=n[t]%s|0,null==r?e<3?(0==e?u=u/100|0:1==e&&(u=u/10|0),o=i<4&&99999==u||i>3&&49999==u||5e4==u||0==u):o=(i<4&&u+1==s||i>3&&u+1==s/2)&&(n[t+1]/s/100|0)==qn(10,e-2)-1||(u==s/2||0==u)&&0==(n[t+1]/s/100|0):e<4?(0==e?u=u/1e3|0:1==e?u=u/100|0:2==e&&(u=u/10|0),o=(r||i<4)&&9999==u||!r&&i>3&&4999==u):o=((r||i<4)&&u+1==s||!r&&i>3&&u+1==s/2)&&(n[t+1]/s/1e3|0)==qn(10,e-3)-1,o}function t(n,e,i){for(var r,t,s=[0],o=0,u=n.length;o<u;){for(t=s.length;t--;)s[t]*=e;for(s[0]+=gn.indexOf(n.charAt(o++)),r=0;r<s.length;r++)s[r]>i-1&&(void 0===s[r+1]&&(s[r+1]=0),s[r+1]+=s[r]/i|0,s[r]%=i)}return s.reverse()}function s(n,e){var i,r,t=e.d.length;t<32?(i=Math.ceil(t/3),r=Math.pow(4,-i).toString()):(i=16,r="2.3283064365386962890625e-10"),n.precision+=i,e=x(n,1,e.times(r),new n(1));for(var s=i;s--;){var o=e.times(e);e=o.times(o).minus(o).times(8).plus(1)}return n.precision-=i,e}function o(n,e,i,r){var t,s,o,u,c,f,h,a,d,l=n.constructor;n:if(null!=e){if(!(a=n.d))return n;for(t=1,u=a[0];u>=10;u/=10)t++;if((s=e-t)<0)s+=Pn,o=e,c=(h=a[d=0])/qn(10,t-o-1)%10|0;else if(d=Math.ceil((s+1)/Pn),u=a.length,d>=u){if(!r)break n;for(;u++<=d;)a.push(0);h=c=0,t=1,o=(s%=Pn)-Pn+1}else{for(h=u=a[d],t=1;u>=10;u/=10)t++;c=(o=(s%=Pn)-Pn+t)<0?0:h/qn(10,t-o-1)%10|0}if(r=r||e<0||void 0!==a[d+1]||(o<0?h:h%qn(10,t-o-1)),f=i<4?(c||r)&&(0==i||i==(n.s<0?3:2)):c>5||5==c&&(4==i||r||6==i&&(s>0?o>0?h/qn(10,t-o):0:a[d-1])%10&1||i==(n.s<0?8:7)),e<1||!a[0])return a.length=0,f?(e-=n.e+1,a[0]=qn(10,(Pn-e%Pn)%Pn),n.e=-e||0):a[0]=n.e=0,n;if(0==s?(a.length=d,u=1,d--):(a.length=d+1,u=qn(10,Pn-s),a[d]=o>0?(h/qn(10,t-o)%qn(10,o)|0)*u:0),f)for(;;){if(0==d){for(s=1,o=a[0];o>=10;o/=10)s++;for(o=a[0]+=u,u=1;o>=10;o/=10)u++;s!=u&&(n.e++,a[0]==Dn&&(a[0]=1));break}if(a[d]+=u,a[d]!=Dn)break;a[d--]=0,u=1}for(s=a.length;0===a[--s];)a.pop()}return Nn&&(n.e>l.maxE?(n.d=null,n.e=NaN):n.e<l.minE&&(n.e=0,n.d=[0])),n}function u(n,i,r){if(!n.isFinite())return v(n);var t,s=n.e,o=e(n.d),u=o.length;return i?(r&&(t=r-u)>0?o=o.charAt(0)+"."+o.slice(1)+d(t):u>1&&(o=o.charAt(0)+"."+o.slice(1)),o=o+(n.e<0?"e":"e+")+n.e):s<0?(o="0."+d(-s-1)+o,r&&(t=r-u)>0&&(o+=d(t))):s>=u?(o+=d(s+1-u),r&&(t=r-s-1)>0&&(o=o+"."+d(t))):((t=s+1)<u&&(o=o.slice(0,t)+"."+o.slice(t)),r&&(t=r-u)>0&&(s+1===u&&(o+="."),o+=d(t))),o}function c(n,e){var i=n[0];for(e*=Pn;i>=10;i/=10)e++;return e}function f(n,e,i){if(e>Rn)throw Nn=!0,i&&(n.precision=i),Error(xn);return o(new n(wn),e,1,!0)}function h(n,e,i){if(e>Ln)throw Error(xn);return o(new n(mn),e,i,!0)}function a(n){var e=n.length-1,i=e*Pn+1;if(e=n[e]){for(;e%10==0;e/=10)i--;for(e=n[0];e>=10;e/=10)i++}return i}function d(n){for(var e="";n--;)e+="0";return e}function l(n,e,i,r){var t,s=new n(1),o=Math.ceil(r/Pn+4);for(Nn=!1;;){if(i%2&&q((s=s.times(e)).d,o)&&(t=!0),0===(i=yn(i/2))){i=s.d.length-1,t&&0===s.d[i]&&++s.d[i];break}q((e=e.times(e)).d,o)}return Nn=!0,s}function p(n){return 1&n.d[n.d.length-1]}function g(n,e,i){for(var r,t=new n(e[0]),s=0;++s<e.length;){if(!(r=new n(e[s])).s){t=r;break}t[i](r)&&(t=r)}return t}function w(n,i){var t,s,u,c,f,h,a,d=0,l=0,p=0,g=n.constructor,w=g.rounding,m=g.precision;if(!n.d||!n.d[0]||n.e>17)return new g(n.d?n.d[0]?n.s<0?0:1/0:1:n.s?n.s<0?0:n:NaN);for(null==i?(Nn=!1,a=m):a=i,h=new g(.03125);n.e>-2;)n=n.times(h),p+=5;for(a+=s=Math.log(qn(2,p))/Math.LN10*2+5|0,t=c=f=new g(1),g.precision=a;;){if(c=o(c.times(n),a,1),t=t.times(++l),h=f.plus(_n(c,t,a,1)),e(h.d).slice(0,a)===e(f.d).slice(0,a)){for(u=p;u--;)f=o(f.times(f),a,1);if(null!=i)return g.precision=m,f;if(!(d<3&&r(f.d,a-s,w,d)))return o(f,g.precision=m,w,Nn=!0);g.precision=a+=10,t=c=h=new g(1),l=0,d++}f=h}}function m(n,i){var t,s,u,c,h,a,d,l,p,g,w,v=1,N=n,b=N.d,E=N.constructor,x=E.rounding,M=E.precision;if(N.s<0||!b||!b[0]||!N.e&&1==b[0]&&1==b.length)return new E(b&&!b[0]?-1/0:1!=N.s?NaN:b?0:N);if(null==i?(Nn=!1,p=M):p=i,E.precision=p+=10,t=e(b),s=t.charAt(0),!(Math.abs(c=N.e)<15e14))return l=f(E,p+2,M).times(c+""),N=m(new E(s+"."+t.slice(1)),p-10).plus(l),E.precision=M,null==i?o(N,M,x,Nn=!0):N;for(;s<7&&1!=s||1==s&&t.charAt(1)>3;)s=(t=e((N=N.times(n)).d)).charAt(0),v++;for(c=N.e,s>1?(N=new E("0."+t),c++):N=new E(s+"."+t.slice(1)),g=N,d=h=N=_n(N.minus(1),N.plus(1),p,1),w=o(N.times(N),p,1),u=3;;){if(h=o(h.times(w),p,1),l=d.plus(_n(h,new E(u),p,1)),e(l.d).slice(0,p)===e(d.d).slice(0,p)){if(d=d.times(2),0!==c&&(d=d.plus(f(E,p+2,M).times(c+""))),d=_n(d,new E(v),p,1),null!=i)return E.precision=M,d;if(!r(d.d,p-10,x,a))return o(d,E.precision=M,x,Nn=!0);E.precision=p+=10,l=h=N=_n(g.minus(1),g.plus(1),p,1),w=o(N.times(N),p,1),u=a=1}d=l,u+=2}}function v(n){return String(n.s*n.s/0)}function N(n,e){var i,r,t;for((i=e.indexOf("."))>-1&&(e=e.replace(".","")),(r=e.search(/e/i))>0?(i<0&&(i=r),i+=+e.slice(r+1),e=e.substring(0,r)):i<0&&(i=e.length),r=0;48===e.charCodeAt(r);r++);for(t=e.length;48===e.charCodeAt(t-1);--t);if(e=e.slice(r,t)){if(t-=r,n.e=i=i-r-1,n.d=[],r=(i+1)%Pn,i<0&&(r+=Pn),r<t){for(r&&n.d.push(+e.slice(0,r)),t-=Pn;r<t;)n.d.push(+e.slice(r,r+=Pn));e=e.slice(r),r=Pn-e.length}else r-=t;for(;r--;)e+="0";n.d.push(+e),Nn&&(n.e>n.constructor.maxE?(n.d=null,n.e=NaN):n.e<n.constructor.minE&&(n.e=0,n.d=[0]))}else n.e=0,n.d=[0];return n}function b(n,e){var i,r,s,o,u,f,h,a,d;if("Infinity"===e||"NaN"===e)return+e||(n.s=NaN),n.e=NaN,n.d=null,n;if(Fn.test(e))i=16,e=e.toLowerCase();else if(On.test(e))i=2;else{if(!An.test(e))throw Error(En+e);i=8}for((o=e.search(/p/i))>0?(h=+e.slice(o+1),e=e.substring(2,o)):e=e.slice(2),u=(o=e.indexOf("."))>=0,r=n.constructor,u&&(o=(f=(e=e.replace(".","")).length)-o,s=l(r,new r(i),o,2*o)),o=d=(a=t(e,i,Dn)).length-1;0===a[o];--o)a.pop();return o<0?new r(0*n.s):(n.e=c(a,d),n.d=a,Nn=!1,u&&(n=_n(n,s,4*f)),h&&(n=n.times(Math.abs(h)<54?Math.pow(2,h):vn.pow(2,h))),Nn=!0,n)}function E(n,e){var i,r=e.d.length;if(r<3)return x(n,2,e,e);i=(i=1.4*Math.sqrt(r))>16?16:0|i,e=x(n,2,e=e.times(Math.pow(5,-i)),e);for(var t,s=new n(5),o=new n(16),u=new n(20);i--;)t=e.times(e),e=e.times(s.plus(t.times(o.times(t).minus(u))));return e}function x(n,e,i,r,t){var s,o,u,c,f=1,h=n.precision,a=Math.ceil(h/Pn);for(Nn=!1,c=i.times(i),u=new n(r);;){if(o=_n(u.times(c),new n(e++*e++),h,1),u=t?r.plus(o):r.minus(o),r=_n(o.times(c),new n(e++*e++),h,1),void 0!==(o=u.plus(r)).d[a]){for(s=a;o.d[s]===u.d[s]&&s--;);if(-1==s)break}s=u,u=r,r=o,o=s,f++}return Nn=!0,o.d.length=a+1,o}function M(n,e){var i,r=e.s<0,t=h(n,n.precision,1),s=t.times(.5);if((e=e.abs()).lte(s))return dn=r?4:1,e;if((i=e.divToInt(t)).isZero())dn=r?3:2;else{if((e=e.minus(i.times(t))).lte(s))return dn=p(i)?r?2:3:r?4:1,e;dn=p(i)?r?1:4:r?3:2}return e.minus(t).abs()}function y(n,e,r,s){var o,c,f,h,a,d,l,p,g,w=n.constructor,m=void 0!==r;if(m?(i(r,1,pn),void 0===s?s=w.rounding:i(s,0,8)):(r=w.precision,s=w.rounding),n.isFinite()){for(f=(l=u(n)).indexOf("."),m?(o=2,16==e?r=4*r-3:8==e&&(r=3*r-2)):o=e,f>=0&&(l=l.replace(".",""),(g=new w(1)).e=l.length-f,g.d=t(u(g),10,o),g.e=g.d.length),c=a=(p=t(l,10,o)).length;0==p[--a];)p.pop();if(p[0]){if(f<0?c--:((n=new w(n)).d=p,n.e=c,p=(n=_n(n,g,r,s,0,o)).d,c=n.e,d=an),f=p[r],h=o/2,d=d||void 0!==p[r+1],d=s<4?(void 0!==f||d)&&(0===s||s===(n.s<0?3:2)):f>h||f===h&&(4===s||d||6===s&&1&p[r-1]||s===(n.s<0?8:7)),p.length=r,d)for(;++p[--r]>o-1;)p[r]=0,r||(++c,p.unshift(1));for(a=p.length;!p[a-1];--a);for(f=0,l="";f<a;f++)l+=gn.charAt(p[f]);if(m){if(a>1)if(16==e||8==e){for(f=16==e?4:3,--a;a%f;a++)l+="0";for(a=(p=t(l,o,e)).length;!p[a-1];--a);for(f=1,l="1.";f<a;f++)l+=gn.charAt(p[f])}else l=l.charAt(0)+"."+l.slice(1);l=l+(c<0?"p":"p+")+c}else if(c<0){for(;++c;)l="0"+l;l="0."+l}else if(++c>a)for(c-=a;c--;)l+="0";else c<a&&(l=l.slice(0,c)+"."+l.slice(c))}else l=m?"0p+0":"0";l=(16==e?"0x":2==e?"0b":8==e?"0o":"")+l}else l=v(n);return n.s<0?"-"+l:l}function q(n,e){if(n.length>e)return n.length=e,!0}function O(n){return new this(n).abs()}function F(n){return new this(n).acos()}function A(n){return new this(n).acosh()}function Z(n,e){return new this(n).plus(e)}function D(n){return new this(n).asin()}function P(n){return new this(n).asinh()}function R(n){return new this(n).atan()}function L(n){return new this(n).atanh()}function U(n,e){n=new this(n),e=new this(e);var i,r=this.precision,t=this.rounding,s=r+4;return n.s&&e.s?n.d||e.d?!e.d||n.isZero()?(i=e.s<0?h(this,r,t):new this(0)).s=n.s:!n.d||e.isZero()?(i=h(this,s,1).times(.5)).s=n.s:e.s<0?(this.precision=s,this.rounding=1,i=this.atan(_n(n,e,s,1)),e=h(this,s,1),this.precision=r,this.rounding=t,i=n.s<0?i.minus(e):i.plus(e)):i=this.atan(_n(n,e,s,1)):(i=h(this,s,1).times(e.s>0?.25:.75)).s=n.s:i=new this(NaN),i}function _(n){return new this(n).cbrt()}function k(n){return o(n=new this(n),n.e+1,2)}function S(n){if(!n||"object"!=typeof n)throw Error(bn+"Object expected");var e,i,r,t=["precision",1,pn,"rounding",0,8,"toExpNeg",-ln,0,"toExpPos",0,ln,"maxE",0,ln,"minE",-ln,0,"modulo",0,9];for(e=0;e<t.length;e+=3)if(void 0!==(r=n[i=t[e]])){if(!(yn(r)===r&&r>=t[e+1]&&r<=t[e+2]))throw Error(En+i+": "+r);this[i]=r}if(void 0!==(r=n[i="crypto"])){if(!0!==r&&!1!==r&&0!==r&&1!==r)throw Error(En+i+": "+r);if(r){if("undefined"==typeof crypto||!crypto||!crypto.getRandomValues&&!crypto.randomBytes)throw Error(Mn);this[i]=!0}else this[i]=!1}return this}function T(n){return new this(n).cos()}function I(n){return new this(n).cosh()}function C(n){function e(n){var i,r,t,s=this;if(!(s instanceof e))return new e(n);if(s.constructor=e,n instanceof e)return s.s=n.s,s.e=n.e,void(s.d=(n=n.d)?n.slice():n);if("number"==(t=typeof n)){if(0===n)return s.s=1/n<0?-1:1,s.e=0,void(s.d=[0]);if(n<0?(n=-n,s.s=-1):s.s=1,n===~~n&&n<1e7){for(i=0,r=n;r>=10;r/=10)i++;return s.e=i,void(s.d=[n])}return 0*n!=0?(n||(s.s=NaN),s.e=NaN,void(s.d=null)):N(s,n.toString())}if("string"!==t)throw Error(En+n);return 45===n.charCodeAt(0)?(n=n.slice(1),s.s=-1):s.s=1,Zn.test(n)?N(s,n):b(s,n)}var i,r,t;if(e.prototype=Un,e.ROUND_UP=0,e.ROUND_DOWN=1,e.ROUND_CEIL=2,e.ROUND_FLOOR=3,e.ROUND_HALF_UP=4,e.ROUND_HALF_DOWN=5,e.ROUND_HALF_EVEN=6,e.ROUND_HALF_CEIL=7,e.ROUND_HALF_FLOOR=8,e.EUCLID=9,e.config=e.set=S,e.clone=C,e.abs=O,e.acos=F,e.acosh=A,e.add=Z,e.asin=D,e.asinh=P,e.atan=R,e.atanh=L,e.atan2=U,e.cbrt=_,e.ceil=k,e.cos=T,e.cosh=I,e.div=H,e.exp=B,e.floor=V,e.hypot=$,e.ln=j,e.log=W,e.log10=z,e.log2=J,e.max=G,e.min=K,e.mod=Q,e.mul=X,e.pow=Y,e.random=nn,e.round=en,e.sign=rn,e.sin=tn,e.sinh=sn,e.sqrt=on,e.sub=un,e.tan=cn,e.tanh=fn,e.trunc=hn,void 0===n&&(n={}),n)for(t=["precision","rounding","toExpNeg","toExpPos","maxE","minE","modulo","crypto"],i=0;i<t.length;)n.hasOwnProperty(r=t[i++])||(n[r]=this[r]);return e.config(n),e}function H(n,e){return new this(n).div(e)}function B(n){return new this(n).exp()}function V(n){return o(n=new this(n),n.e+1,3)}function $(){var n,e,i=new this(0);for(Nn=!1,n=0;n<arguments.length;)if((e=new this(arguments[n++])).d)i.d&&(i=i.plus(e.times(e)));else{if(e.s)return Nn=!0,new this(1/0);i=e}return Nn=!0,i.sqrt()}function j(n){return new this(n).ln()}function W(n,e){return new this(n).log(e)}function J(n){return new this(n).log(2)}function z(n){return new this(n).log(10)}function G(){return g(this,arguments,"lt")}function K(){return g(this,arguments,"gt")}function Q(n,e){return new this(n).mod(e)}function X(n,e){return new this(n).mul(e)}function Y(n,e){return new this(n).pow(e)}function nn(n){var e,r,t,s,o=0,u=new this(1),c=[];if(void 0===n?n=this.precision:i(n,1,pn),t=Math.ceil(n/Pn),this.crypto)if(crypto.getRandomValues)for(e=crypto.getRandomValues(new Uint32Array(t));o<t;)(s=e[o])>=429e7?e[o]=crypto.getRandomValues(new Uint32Array(1))[0]:c[o++]=s%1e7;else{if(!crypto.randomBytes)throw Error(Mn);for(e=crypto.randomBytes(t*=4);o<t;)(s=e[o]+(e[o+1]<<8)+(e[o+2]<<16)+((127&e[o+3])<<24))>=214e7?crypto.randomBytes(4).copy(e,o):(c.push(s%1e7),o+=4);o=t/4}else for(;o<t;)c[o++]=1e7*Math.random()|0;for(t=c[--o],n%=Pn,t&&n&&(s=qn(10,Pn-n),c[o]=(t/s|0)*s);0===c[o];o--)c.pop();if(o<0)r=0,c=[0];else{for(r=-1;0===c[0];r-=Pn)c.shift();for(t=1,s=c[0];s>=10;s/=10)t++;t<Pn&&(r-=Pn-t)}return u.e=r,u.d=c,u}function en(n){return o(n=new this(n),n.e+1,this.rounding)}function rn(n){return n=new this(n),n.d?n.d[0]?n.s:0*n.s:n.s||NaN}function tn(n){return new this(n).sin()}function sn(n){return new this(n).sinh()}function on(n){return new this(n).sqrt()}function un(n,e){return new this(n).sub(e)}function cn(n){return new this(n).tan()}function fn(n){return new this(n).tanh()}function hn(n){return o(n=new this(n),n.e+1,1)}var an,dn,ln=9e15,pn=1e9,gn="0123456789abcdef",wn="2.",mn="3.",vn={precision:20,rounding:4,modulo:1,toExpNeg:-7,toExpPos:21,minE:-ln,maxE:ln,crypto:!1},Nn=!0,bn="[DecimalError] ",En=bn+"Invalid argument: ",xn=bn+"Precision limit exceeded",Mn=bn+"crypto unavailable",yn=Math.floor,qn=Math.pow,On=/^0b([01]+(\.[01]*)?|\.[01]+)(p[+-]?\d+)?$/i,Fn=/^0x([0-9a-f]+(\.[0-9a-f]*)?|\.[0-9a-f]+)(p[+-]?\d+)?$/i,An=/^0o([0-7]+(\.[0-7]*)?|\.[0-7]+)(p[+-]?\d+)?$/i,Zn=/^(\d+(\.\d*)?|\.\d+)(e[+-]?\d+)?$/i,Dn=1e7,Pn=7,Rn=wn.length-1,Ln=mn.length-1,Un={};Un.absoluteValue=Un.abs=function(){var n=new this.constructor(this);return n.s<0&&(n.s=1),o(n)},Un.ceil=function(){return o(new this.constructor(this),this.e+1,2)},Un.comparedTo=Un.cmp=function(n){var e,i,r,t,s=this,o=s.d,u=(n=new s.constructor(n)).d,c=s.s,f=n.s;if(!o||!u)return c&&f?c!==f?c:o===u?0:!o^c<0?1:-1:NaN;if(!o[0]||!u[0])return o[0]?c:u[0]?-f:0;if(c!==f)return c;if(s.e!==n.e)return s.e>n.e^c<0?1:-1;for(e=0,i=(r=o.length)<(t=u.length)?r:t;e<i;++e)if(o[e]!==u[e])return o[e]>u[e]^c<0?1:-1;return r===t?0:r>t^c<0?1:-1},Un.cosine=Un.cos=function(){var n,e,i=this,r=i.constructor;return i.d?i.d[0]?(n=r.precision,e=r.rounding,r.precision=n+Math.max(i.e,i.sd())+Pn,r.rounding=1,i=s(r,M(r,i)),r.precision=n,r.rounding=e,o(2==dn||3==dn?i.neg():i,n,e,!0)):new r(1):new r(NaN)},Un.cubeRoot=Un.cbrt=function(){var n,i,r,t,s,u,c,f,h,a,d=this,l=d.constructor;if(!d.isFinite()||d.isZero())return new l(d);for(Nn=!1,(u=d.s*Math.pow(d.s*d,1/3))&&Math.abs(u)!=1/0?t=new l(u.toString()):(r=e(d.d),(u=((n=d.e)-r.length+1)%3)&&(r+=1==u||-2==u?"0":"00"),u=Math.pow(r,1/3),n=yn((n+1)/3)-(n%3==(n<0?-1:2)),(t=new l(r=u==1/0?"5e"+n:(r=u.toExponential()).slice(0,r.indexOf("e")+1)+n)).s=d.s),c=(n=l.precision)+3;;)if(f=t,h=f.times(f).times(f),a=h.plus(d),t=_n(a.plus(d).times(f),a.plus(h),c+2,1),e(f.d).slice(0,c)===(r=e(t.d)).slice(0,c)){if("9999"!=(r=r.slice(c-3,c+1))&&(s||"4999"!=r)){+r&&(+r.slice(1)||"5"!=r.charAt(0))||(o(t,n+1,1),i=!t.times(t).times(t).eq(d));break}if(!s&&(o(f,n+1,0),f.times(f).times(f).eq(d))){t=f;break}c+=4,s=1}return Nn=!0,o(t,n,l.rounding,i)},Un.decimalPlaces=Un.dp=function(){var n,e=this.d,i=NaN;if(e){if(n=e.length-1,i=(n-yn(this.e/Pn))*Pn,n=e[n])for(;n%10==0;n/=10)i--;i<0&&(i=0)}return i},Un.dividedBy=Un.div=function(n){return _n(this,new this.constructor(n))},Un.dividedToIntegerBy=Un.divToInt=function(n){var e=this,i=e.constructor;return o(_n(e,new i(n),0,1,1),i.precision,i.rounding)},Un.equals=Un.eq=function(n){return 0===this.cmp(n)},Un.floor=function(){return o(new this.constructor(this),this.e+1,3)},Un.greaterThan=Un.gt=function(n){return this.cmp(n)>0},Un.greaterThanOrEqualTo=Un.gte=function(n){var e=this.cmp(n);return 1==e||0===e},Un.hyperbolicCosine=Un.cosh=function(){var n,e,i,r,t,s=this,u=s.constructor,c=new u(1);if(!s.isFinite())return new u(s.s?1/0:NaN);if(s.isZero())return c;i=u.precision,r=u.rounding,u.precision=i+Math.max(s.e,s.sd())+4,u.rounding=1,(t=s.d.length)<32?(n=Math.ceil(t/3),e=Math.pow(4,-n).toString()):(n=16,e="2.3283064365386962890625e-10"),s=x(u,1,s.times(e),new u(1),!0);for(var f,h=n,a=new u(8);h--;)f=s.times(s),s=c.minus(f.times(a.minus(f.times(a))));return o(s,u.precision=i,u.rounding=r,!0)},Un.hyperbolicSine=Un.sinh=function(){var n,e,i,r,t=this,s=t.constructor;if(!t.isFinite()||t.isZero())return new s(t);if(e=s.precision,i=s.rounding,s.precision=e+Math.max(t.e,t.sd())+4,s.rounding=1,(r=t.d.length)<3)t=x(s,2,t,t,!0);else{n=(n=1.4*Math.sqrt(r))>16?16:0|n,t=x(s,2,t=t.times(Math.pow(5,-n)),t,!0);for(var u,c=new s(5),f=new s(16),h=new s(20);n--;)u=t.times(t),t=t.times(c.plus(u.times(f.times(u).plus(h))))}return s.precision=e,s.rounding=i,o(t,e,i,!0)},Un.hyperbolicTangent=Un.tanh=function(){var n,e,i=this,r=i.constructor;return i.isFinite()?i.isZero()?new r(i):(n=r.precision,e=r.rounding,r.precision=n+7,r.rounding=1,_n(i.sinh(),i.cosh(),r.precision=n,r.rounding=e)):new r(i.s)},Un.inverseCosine=Un.acos=function(){var n,e=this,i=e.constructor,r=e.abs().cmp(1),t=i.precision,s=i.rounding;return-1!==r?0===r?e.isNeg()?h(i,t,s):new i(0):new i(NaN):e.isZero()?h(i,t+4,s).times(.5):(i.precision=t+6,i.rounding=1,e=e.asin(),n=h(i,t+4,s).times(.5),i.precision=t,i.rounding=s,n.minus(e))},Un.inverseHyperbolicCosine=Un.acosh=function(){var n,e,i=this,r=i.constructor;return i.lte(1)?new r(i.eq(1)?0:NaN):i.isFinite()?(n=r.precision,e=r.rounding,r.precision=n+Math.max(Math.abs(i.e),i.sd())+4,r.rounding=1,Nn=!1,i=i.times(i).minus(1).sqrt().plus(i),Nn=!0,r.precision=n,r.rounding=e,i.ln()):new r(i)},Un.inverseHyperbolicSine=Un.asinh=function(){var n,e,i=this,r=i.constructor;return!i.isFinite()||i.isZero()?new r(i):(n=r.precision,e=r.rounding,r.precision=n+2*Math.max(Math.abs(i.e),i.sd())+6,r.rounding=1,Nn=!1,i=i.times(i).plus(1).sqrt().plus(i),Nn=!0,r.precision=n,r.rounding=e,i.ln())},Un.inverseHyperbolicTangent=Un.atanh=function(){var n,e,i,r,t=this,s=t.constructor;return t.isFinite()?t.e>=0?new s(t.abs().eq(1)?t.s/0:t.isZero()?t:NaN):(n=s.precision,e=s.rounding,r=t.sd(),Math.max(r,n)<2*-t.e-1?o(new s(t),n,e,!0):(s.precision=i=r-t.e,t=_n(t.plus(1),new s(1).minus(t),i+n,1),s.precision=n+4,s.rounding=1,t=t.ln(),s.precision=n,s.rounding=e,t.times(.5))):new s(NaN)},Un.inverseSine=Un.asin=function(){var n,e,i,r,t=this,s=t.constructor;return t.isZero()?new s(t):(e=t.abs().cmp(1),i=s.precision,r=s.rounding,-1!==e?0===e?(n=h(s,i+4,r).times(.5),n.s=t.s,n):new s(NaN):(s.precision=i+6,s.rounding=1,t=t.div(new s(1).minus(t.times(t)).sqrt().plus(1)).atan(),s.precision=i,s.rounding=r,t.times(2)))},Un.inverseTangent=Un.atan=function(){var n,e,i,r,t,s,u,c,f,a=this,d=a.constructor,l=d.precision,p=d.rounding;if(a.isFinite()){if(a.isZero())return new d(a);if(a.abs().eq(1)&&l+4<=Ln)return u=h(d,l+4,p).times(.25),u.s=a.s,u}else{if(!a.s)return new d(NaN);if(l+4<=Ln)return u=h(d,l+4,p).times(.5),u.s=a.s,u}for(d.precision=c=l+10,d.rounding=1,n=i=Math.min(28,c/Pn+2|0);n;--n)a=a.div(a.times(a).plus(1).sqrt().plus(1));for(Nn=!1,e=Math.ceil(c/Pn),r=1,f=a.times(a),u=new d(a),t=a;-1!==n;)if(t=t.times(f),s=u.minus(t.div(r+=2)),t=t.times(f),void 0!==(u=s.plus(t.div(r+=2))).d[e])for(n=e;u.d[n]===s.d[n]&&n--;);return i&&(u=u.times(2<<i-1)),Nn=!0,o(u,d.precision=l,d.rounding=p,!0)},Un.isFinite=function(){return!!this.d},Un.isInteger=Un.isInt=function(){return!!this.d&&yn(this.e/Pn)>this.d.length-2},Un.isNaN=function(){return!this.s},Un.isNegative=Un.isNeg=function(){return this.s<0},Un.isPositive=Un.isPos=function(){return this.s>0},Un.isZero=function(){return!!this.d&&0===this.d[0]},Un.lessThan=Un.lt=function(n){return this.cmp(n)<0},Un.lessThanOrEqualTo=Un.lte=function(n){return this.cmp(n)<1},Un.logarithm=Un.log=function(n){var i,t,s,u,c,h,a,d,l=this,p=l.constructor,g=p.precision,w=p.rounding;if(null==n)n=new p(10),i=!0;else{if(n=new p(n),t=n.d,n.s<0||!t||!t[0]||n.eq(1))return new p(NaN);i=n.eq(10)}if(t=l.d,l.s<0||!t||!t[0]||l.eq(1))return new p(t&&!t[0]?-1/0:1!=l.s?NaN:t?0:1/0);if(i)if(t.length>1)c=!0;else{for(u=t[0];u%10==0;)u/=10;c=1!==u}if(Nn=!1,a=g+5,h=m(l,a),s=i?f(p,a+10):m(n,a),d=_n(h,s,a,1),r(d.d,u=g,w))do{if(a+=10,h=m(l,a),s=i?f(p,a+10):m(n,a),d=_n(h,s,a,1),!c){+e(d.d).slice(u+1,u+15)+1==1e14&&(d=o(d,g+1,0));break}}while(r(d.d,u+=10,w));return Nn=!0,o(d,g,w)},Un.minus=Un.sub=function(n){var e,i,r,t,s,u,f,h,a,d,l,p,g=this,w=g.constructor;if(n=new w(n),!g.d||!n.d)return g.s&&n.s?g.d?n.s=-n.s:n=new w(n.d||g.s!==n.s?g:NaN):n=new w(NaN),n;if(g.s!=n.s)return n.s=-n.s,g.plus(n);if(a=g.d,p=n.d,f=w.precision,h=w.rounding,!a[0]||!p[0]){if(p[0])n.s=-n.s;else{if(!a[0])return new w(3===h?-0:0);n=new w(g)}return Nn?o(n,f,h):n}if(i=yn(n.e/Pn),d=yn(g.e/Pn),a=a.slice(),s=d-i){for((l=s<0)?(e=a,s=-s,u=p.length):(e=p,i=d,u=a.length),s>(r=Math.max(Math.ceil(f/Pn),u)+2)&&(s=r,e.length=1),e.reverse(),r=s;r--;)e.push(0);e.reverse()}else{for((l=(r=a.length)<(u=p.length))&&(u=r),r=0;r<u;r++)if(a[r]!=p[r]){l=a[r]<p[r];break}s=0}for(l&&(e=a,a=p,p=e,n.s=-n.s),u=a.length,r=p.length-u;r>0;--r)a[u++]=0;for(r=p.length;r>s;){if(a[--r]<p[r]){for(t=r;t&&0===a[--t];)a[t]=Dn-1;--a[t],a[r]+=Dn}a[r]-=p[r]}for(;0===a[--u];)a.pop();for(;0===a[0];a.shift())--i;return a[0]?(n.d=a,n.e=c(a,i),Nn?o(n,f,h):n):new w(3===h?-0:0)},Un.modulo=Un.mod=function(n){var e,i=this,r=i.constructor;return n=new r(n),!i.d||!n.s||n.d&&!n.d[0]?new r(NaN):!n.d||i.d&&!i.d[0]?o(new r(i),r.precision,r.rounding):(Nn=!1,9==r.modulo?(e=_n(i,n.abs(),0,3,1)).s*=n.s:e=_n(i,n,0,r.modulo,1),e=e.times(n),Nn=!0,i.minus(e))},Un.naturalExponential=Un.exp=function(){return w(this)},Un.naturalLogarithm=Un.ln=function(){return m(this)},Un.negated=Un.neg=function(){var n=new this.constructor(this);return n.s=-n.s,o(n)},Un.plus=Un.add=function(n){var e,i,r,t,s,u,f,h,a,d,l=this,p=l.constructor;if(n=new p(n),!l.d||!n.d)return l.s&&n.s?l.d||(n=new p(n.d||l.s===n.s?l:NaN)):n=new p(NaN),n;if(l.s!=n.s)return n.s=-n.s,l.minus(n);if(a=l.d,d=n.d,f=p.precision,h=p.rounding,!a[0]||!d[0])return d[0]||(n=new p(l)),Nn?o(n,f,h):n;if(s=yn(l.e/Pn),r=yn(n.e/Pn),a=a.slice(),t=s-r){for(t<0?(i=a,t=-t,u=d.length):(i=d,r=s,u=a.length),t>(u=(s=Math.ceil(f/Pn))>u?s+1:u+1)&&(t=u,i.length=1),i.reverse();t--;)i.push(0);i.reverse()}for((u=a.length)-(t=d.length)<0&&(t=u,i=d,d=a,a=i),e=0;t;)e=(a[--t]=a[t]+d[t]+e)/Dn|0,a[t]%=Dn;for(e&&(a.unshift(e),++r),u=a.length;0==a[--u];)a.pop();return n.d=a,n.e=c(a,r),Nn?o(n,f,h):n},Un.precision=Un.sd=function(n){var e,i=this;if(void 0!==n&&n!==!!n&&1!==n&&0!==n)throw Error(En+n);return i.d?(e=a(i.d),n&&i.e+1>e&&(e=i.e+1)):e=NaN,e},Un.round=function(){var n=this,e=n.constructor;return o(new e(n),n.e+1,e.rounding)},Un.sine=Un.sin=function(){var n,e,i=this,r=i.constructor;return i.isFinite()?i.isZero()?new r(i):(n=r.precision,e=r.rounding,r.precision=n+Math.max(i.e,i.sd())+Pn,r.rounding=1,i=E(r,M(r,i)),r.precision=n,r.rounding=e,o(dn>2?i.neg():i,n,e,!0)):new r(NaN)},Un.squareRoot=Un.sqrt=function(){var n,i,r,t,s,u,c=this,f=c.d,h=c.e,a=c.s,d=c.constructor;if(1!==a||!f||!f[0])return new d(!a||a<0&&(!f||f[0])?NaN:f?c:1/0);for(Nn=!1,0==(a=Math.sqrt(+c))||a==1/0?(((i=e(f)).length+h)%2==0&&(i+="0"),a=Math.sqrt(i),h=yn((h+1)/2)-(h<0||h%2),t=new d(i=a==1/0?"1e"+h:(i=a.toExponential()).slice(0,i.indexOf("e")+1)+h)):t=new d(a.toString()),r=(h=d.precision)+3;;)if(u=t,t=u.plus(_n(c,u,r+2,1)).times(.5),e(u.d).slice(0,r)===(i=e(t.d)).slice(0,r)){if("9999"!=(i=i.slice(r-3,r+1))&&(s||"4999"!=i)){+i&&(+i.slice(1)||"5"!=i.charAt(0))||(o(t,h+1,1),n=!t.times(t).eq(c));break}if(!s&&(o(u,h+1,0),u.times(u).eq(c))){t=u;break}r+=4,s=1}return Nn=!0,o(t,h,d.rounding,n)},Un.tangent=Un.tan=function(){var n,e,i=this,r=i.constructor;return i.isFinite()?i.isZero()?new r(i):(n=r.precision,e=r.rounding,r.precision=n+10,r.rounding=1,i=i.sin(),i.s=1,i=_n(i,new r(1).minus(i.times(i)).sqrt(),n+10,0),r.precision=n,r.rounding=e,o(2==dn||4==dn?i.neg():i,n,e,!0)):new r(NaN)},Un.times=Un.mul=function(n){var e,i,r,t,s,u,f,h,a,d=this,l=d.constructor,p=d.d,g=(n=new l(n)).d;if(n.s*=d.s,!(p&&p[0]&&g&&g[0]))return new l(!n.s||p&&!p[0]&&!g||g&&!g[0]&&!p?NaN:p&&g?0*n.s:n.s/0);for(i=yn(d.e/Pn)+yn(n.e/Pn),(h=p.length)<(a=g.length)&&(s=p,p=g,g=s,u=h,h=a,a=u),s=[],r=u=h+a;r--;)s.push(0);for(r=a;--r>=0;){for(e=0,t=h+r;t>r;)f=s[t]+g[r]*p[t-r-1]+e,s[t--]=f%Dn|0,e=f/Dn|0;s[t]=(s[t]+e)%Dn|0}for(;!s[--u];)s.pop();return e?++i:s.shift(),n.d=s,n.e=c(s,i),Nn?o(n,l.precision,l.rounding):n},Un.toBinary=function(n,e){return y(this,2,n,e)},Un.toDecimalPlaces=Un.toDP=function(n,e){var r=this,t=r.constructor;return r=new t(r),void 0===n?r:(i(n,0,pn),void 0===e?e=t.rounding:i(e,0,8),o(r,n+r.e+1,e))},Un.toExponential=function(n,e){var r,t=this,s=t.constructor;return void 0===n?r=u(t,!0):(i(n,0,pn),void 0===e?e=s.rounding:i(e,0,8),r=u(t=o(new s(t),n+1,e),!0,n+1)),t.isNeg()&&!t.isZero()?"-"+r:r},Un.toFixed=function(n,e){var r,t,s=this,c=s.constructor;return void 0===n?r=u(s):(i(n,0,pn),void 0===e?e=c.rounding:i(e,0,8),r=u(t=o(new c(s),n+s.e+1,e),!1,n+t.e+1)),s.isNeg()&&!s.isZero()?"-"+r:r},Un.toFraction=function(n){var i,r,t,s,o,u,c,f,h,d,l,p,g=this,w=g.d,m=g.constructor;if(!w)return new m(g);if(h=r=new m(1),t=f=new m(0),i=new m(t),o=i.e=a(w)-g.e-1,u=o%Pn,i.d[0]=qn(10,u<0?Pn+u:u),null==n)n=o>0?i:h;else{if(!(c=new m(n)).isInt()||c.lt(h))throw Error(En+c);n=c.gt(i)?o>0?i:h:c}for(Nn=!1,c=new m(e(w)),d=m.precision,m.precision=o=w.length*Pn*2;l=_n(c,i,0,1,1),1!=(s=r.plus(l.times(t))).cmp(n);)r=t,t=s,s=h,h=f.plus(l.times(s)),f=s,s=i,i=c.minus(l.times(s)),c=s;return s=_n(n.minus(r),t,0,1,1),f=f.plus(s.times(h)),r=r.plus(s.times(t)),f.s=h.s=g.s,p=_n(h,t,o,1).minus(g).abs().cmp(_n(f,r,o,1).minus(g).abs())<1?[h,t]:[f,r],m.precision=d,Nn=!0,p},Un.toHexadecimal=Un.toHex=function(n,e){return y(this,16,n,e)},Un.toNearest=function(n,e){var r=this,t=r.constructor;if(r=new t(r),null==n){if(!r.d)return r;n=new t(1),e=t.rounding}else{if(n=new t(n),void 0!==e&&i(e,0,8),!r.d)return n.s?r:n;if(!n.d)return n.s&&(n.s=r.s),n}return n.d[0]?(Nn=!1,e<4&&(e=[4,5,7,8][e]),r=_n(r,n,0,e,1).times(n),Nn=!0,o(r)):(n.s=r.s,r=n),r},Un.toNumber=function(){return+this},Un.toOctal=function(n,e){return y(this,8,n,e)},Un.toPower=Un.pow=function(n){var i,t,s,u,c,f,h=this,a=h.constructor,d=+(n=new a(n));if(!(h.d&&n.d&&h.d[0]&&n.d[0]))return new a(qn(+h,d));if((h=new a(h)).eq(1))return h;if(s=a.precision,c=a.rounding,n.eq(1))return o(h,s,c);if((i=yn(n.e/Pn))>=n.d.length-1&&(t=d<0?-d:d)<=9007199254740991)return u=l(a,h,t,s),n.s<0?new a(1).div(u):o(u,s,c);if((f=h.s)<0){if(i<n.d.length-1)return new a(NaN);if(0==(1&n.d[i])&&(f=1),0==h.e&&1==h.d[0]&&1==h.d.length)return h.s=f,h}return t=qn(+h,d),(i=0!=t&&isFinite(t)?new a(t+"").e:yn(d*(Math.log("0."+e(h.d))/Math.LN10+h.e+1)))>a.maxE+1||i<a.minE-1?new a(i>0?f/0:0):(Nn=!1,a.rounding=h.s=1,t=Math.min(12,(i+"").length),(u=w(n.times(m(h,s+t)),s)).d&&r((u=o(u,s+5,1)).d,s,c)&&(i=s+10,+e((u=o(w(n.times(m(h,i+t)),i),i+5,1)).d).slice(s+1,s+15)+1==1e14&&(u=o(u,s+1,0))),u.s=f,Nn=!0,a.rounding=c,o(u,s,c))},Un.toPrecision=function(n,e){var r,t=this,s=t.constructor;return void 0===n?r=u(t,t.e<=s.toExpNeg||t.e>=s.toExpPos):(i(n,1,pn),void 0===e?e=s.rounding:i(e,0,8),r=u(t=o(new s(t),n,e),n<=t.e||t.e<=s.toExpNeg,n)),t.isNeg()&&!t.isZero()?"-"+r:r},Un.toSignificantDigits=Un.toSD=function(n,e){var r=this,t=r.constructor;return void 0===n?(n=t.precision,e=t.rounding):(i(n,1,pn),void 0===e?e=t.rounding:i(e,0,8)),o(new t(r),n,e)},Un.toString=function(){var n=this,e=n.constructor,i=u(n,n.e<=e.toExpNeg||n.e>=e.toExpPos);return n.isNeg()&&!n.isZero()?"-"+i:i},Un.truncated=Un.trunc=function(){return o(new this.constructor(this),this.e+1,1)},Un.valueOf=Un.toJSON=function(){var n=this,e=n.constructor,i=u(n,n.e<=e.toExpNeg||n.e>=e.toExpPos);return n.isNeg()?"-"+i:i};var _n=function(){function n(n,e,i){var r,t=0,s=n.length;for(n=n.slice();s--;)r=n[s]*e+t,n[s]=r%i|0,t=r/i|0;return t&&n.unshift(t),n}function e(n,e,i,r){var t,s;if(i!=r)s=i>r?1:-1;else for(t=s=0;t<i;t++)if(n[t]!=e[t]){s=n[t]>e[t]?1:-1;break}return s}function i(n,e,i,r){for(var t=0;i--;)n[i]-=t,t=n[i]<e[i]?1:0,n[i]=t*r+n[i]-e[i];for(;!n[0]&&n.length>1;)n.shift()}return function(r,t,s,u,c,f){var h,a,d,l,p,g,w,m,v,N,b,E,x,M,y,q,O,F,A,Z,D=r.constructor,P=r.s==t.s?1:-1,R=r.d,L=t.d;if(!(R&&R[0]&&L&&L[0]))return new D(r.s&&t.s&&(R?!L||R[0]!=L[0]:L)?R&&0==R[0]||!L?0*P:P/0:NaN);for(f?(p=1,a=r.e-t.e):(f=Dn,p=Pn,a=yn(r.e/p)-yn(t.e/p)),A=L.length,O=R.length,N=(v=new D(P)).d=[],d=0;L[d]==(R[d]||0);d++);if(L[d]>(R[d]||0)&&a--,null==s?(M=s=D.precision,u=D.rounding):M=c?s+(r.e-t.e)+1:s,M<0)N.push(1),g=!0;else{if(M=M/p+2|0,d=0,1==A){for(l=0,L=L[0],M++;(d<O||l)&&M--;d++)y=l*f+(R[d]||0),N[d]=y/L|0,l=y%L|0;g=l||d<O}else{for((l=f/(L[0]+1)|0)>1&&(L=n(L,l,f),R=n(R,l,f),A=L.length,O=R.length),q=A,E=(b=R.slice(0,A)).length;E<A;)b[E++]=0;(Z=L.slice()).unshift(0),F=L[0],L[1]>=f/2&&++F;do{l=0,(h=e(L,b,A,E))<0?(x=b[0],A!=E&&(x=x*f+(b[1]||0)),(l=x/F|0)>1?(l>=f&&(l=f-1),1==(h=e(w=n(L,l,f),b,m=w.length,E=b.length))&&(l--,i(w,A<m?Z:L,m,f))):(0==l&&(h=l=1),w=L.slice()),(m=w.length)<E&&w.unshift(0),i(b,w,E,f),-1==h&&(h=e(L,b,A,E=b.length))<1&&(l++,i(b,A<E?Z:L,E,f)),E=b.length):0===h&&(l++,b=[0]),N[d++]=l,h&&b[0]?b[E++]=R[q]||0:(b=[R[q]],E=1)}while((q++<O||void 0!==b[0])&&M--);g=void 0!==b[0]}N[0]||N.shift()}if(1==p)v.e=a,an=g;else{for(d=1,l=N[0];l>=10;l/=10)d++;v.e=d+a*p-1,o(v,c?s+v.e+1:s,u,g)}return v}}();vn=C(vn),vn.default=vn.Decimal=vn,wn=new vn(wn),mn=new vn(mn); return vn;}();
    })()
});
