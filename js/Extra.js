    ...
    var __ = core.Extra = {
        ...
        LaPlace: {
            transform: function(symbol, t, s) {
                ...
                if(!sym.contains(u, true)) 
                    retval = _.divide(symbol, s.clone());
                else {
                    //e^(a*t)
                    if(sym.isE()) {
                        var tmp = _.parse('e^(-'+s+'*'+u+')*'+sym);
                        //TODO: I have to find a way to put in into more familar form
                        retval = core.Calculus.integrate(tmp, u).sub(u, 0);
                    }
                }
        ...
        Statistics: {
            ...
            sampleVariance: function() {
                var args = [].slice.call(arguments),
                    ...
                    x_ = __.Statistics.mean.apply(__.Statistics, args),
