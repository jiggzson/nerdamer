...
describe('Trig', () => {
	it('should provide the correct value for cos in all quadrants', () => {
		expect(parser.parse('cos(atan(6/11))').text()).toEqual('(11/157)*157^(1/2)');
		expect(parser.parse('cos(-atan(6/11))').text()).toEqual('(11/157)*157^(1/2)');
		...
		expect(parser.parse('cos(pi/4)').text()).toEqual('(1/2)*2^(1/2)');
		expect(parser.parse('cos(3*pi/4)').text()).toEqual('(-1/2)*2^(1/2)');
		expect(parser.parse('cos(5*pi/4)').text()).toEqual('(-1/2)*2^(1/2)');
		expect(parser.parse('cos(-pi/4)').text()).toEqual('(1/2)*2^(1/2)');
	...
	it('should simplify cos as an even function: cos(-x) = cos(x)', () => {
		expect(parser.parse('cos(-x)').text()).toEqual('cos(x)');
		expect(parser.parse('cos(-2*x)').text()).toEqual('cos(2*x)');
		expect(parser.parse('cos(-a*b)').text()).toEqual('cos(a*b)');
	});

	it('should provide the correct value for sin in all quadrants', () => {
		expect(parser.parse('sin(atan(6/11))').text()).toEqual('(6/157)*157^(1/2)');
		expect(parser.parse('sin(-atan(6/11))').text()).toEqual('(-6/157)*157^(1/2)');
		...
		expect(parser.parse('sin(pi/4)').text()).toEqual('(1/2)*2^(1/2)');
		expect(parser.parse('sin(3*pi/4)').text()).toEqual('(1/2)*2^(1/2)');
		expect(parser.parse('sin(5*pi/4)').text()).toEqual('(-1/2)*2^(1/2)');
		expect(parser.parse('sin(-pi/4)').text()).toEqual('(-1/2)*2^(1/2)');
	...
	it('should simplify sin as an odd function: sin(-x) = -sin(x)', () => {
		expect(parser.parse('sin(-x)').text()).toEqual('-sin(x)');
		expect(parser.parse('sin(-2*x)').text()).toEqual('-sin(2*x)');
		expect(parser.parse('sin(-a*b)').text()).toEqual('-sin(a*b)');
	});
	...
	it('should provide the correct value for tan in all quadrants', () => {
		...
		expect(parser.parse('tan(atan(3/4))').text()).toEqual('3/4');
		expect(parser.parse('tan(-asin(6/11))').text()).toEqual('(-6/85)*85^(1/2)');
		expect(parser.parse('tan(asin(6/11))').text()).toEqual('(6/85)*85^(1/2)');
		expect(parser.parse('tan(acos(6/11))').text()).toEqual('(1/6)*85^(1/2)');
		expect(parser.parse('tan(-acos(6/11))').text()).toEqual('(-1/6)*85^(1/2)');
		expect(parser.parse('tan(pi/6)').text()).toEqual('(1/3)*3^(1/2)');
		expect(parser.parse('tan(5*pi/6)').text()).toEqual('(-1/3)*3^(1/2)');
		expect(parser.parse('tan(7*pi/6)').text()).toEqual('(1/3)*3^(1/2)');
		expect(parser.parse('tan(-17*pi/6)').text()).toEqual('(1/3)*3^(1/2)');
	...
	it('should simplify tan as an odd function: tan(-x) = -tan(x)', () => {
		expect(parser.parse('tan(-x)').text()).toEqual('-tan(x)');
		expect(parser.parse('tan(-2*x)').text()).toEqual('-tan(2*x)');
	});
	...
	it('should provide the correct value for sec in all quadrants', () => {
		...
		expect(parser.parse('sec(pi/6)').text()).toEqual('(2/3)*3^(1/2)');
		expect(parser.parse('sec(5*pi/6)').text()).toEqual('(-2/3)*3^(1/2)');
		expect(parser.parse('sec(7*pi/6)').text()).toEqual('(-2/3)*3^(1/2)');
		expect(parser.parse('sec(-17*pi/6)').text()).toEqual('(-2/3)*3^(1/2)');
	...
	it('should throw for sec at undefined points', () => {
		expect(() => {
			parser.parse('sec(-pi/2)');
		}).toThrow();
		expect(() => {
			parser.parse('sec(5*pi/2)');
		}).toThrow();
	});
	...
	it('should provide the correct value for csc in all quadrants', () => {
		...
		expect(parser.parse('csc(-pi/4)').text()).toEqual('-2^(1/2)');
		expect(parser.parse('csc(pi/3)').text()).toEqual('(2/3)*3^(1/2)');
		expect(parser.parse('csc(2*pi/3)').text()).toEqual('(2/3)*3^(1/2)');
		expect(parser.parse('csc(4*pi/3)').text()).toEqual('(-2/3)*3^(1/2)');
		expect(parser.parse('csc(-2*pi/3)').text()).toEqual('(-2/3)*3^(1/2)');
	});

	it('should throw for csc at undefined points', () => {
		expect(() => {
			parser.parse('csc(2*pi)');
		}).toThrow();
		expect(() => {
			parser.parse('csc(-pi)');
		}).toThrow();
		expect(() => {
			parser.parse('csc(33*pi)');
		}).toThrow();
	});
	...
	it('should provide the correct value for cot in all quadrants', () => {
		...
		expect(parser.parse('cot(-pi/4)').text()).toEqual('-1');
		expect(parser.parse('cot(pi/3)').text()).toEqual('(1/3)*3^(1/2)');
		expect(parser.parse('cot(2*pi/3)').text()).toEqual('(-1/3)*3^(1/2)');
		expect(parser.parse('cot(4*pi/3)').text()).toEqual('(1/3)*3^(1/2)');
		expect(parser.parse('cot(-2*pi/3)').text()).toEqual('(1/3)*3^(1/2)');
	});

	it('should throw for cot at undefined points', () => {
		expect(() => {
			parser.parse('cot(2*pi)');
		}).toThrow();
		expect(() => {
			parser.parse('cot(-pi)');
		}).toThrow();
		expect(() => {
			parser.parse('cot(33*pi)');
		}).toThrow();
	});

	it('should provide the correct value for acos in all quadrants', () => {
		expect(parser.parse('acos(0.5)').text()).toEqual('(1/3)*pi');
	...
	it('should provide the correct value for asin in all quadrants', () => {
		expect(parser.parse('asin(0.5)').text()).toEqual('(1/6)*pi');
	...
	it('should calculate hyperbolic trig values correctly', () => {
		...
		expect(parser.evaluate('sech(5)').text()).toEqual('0.013475282221304557306');
		expect(parser.evaluate('sech(-5)').text()).toEqual('0.013475282221304557306');
		...
		expect(parser.evaluate('csch(5)').text()).toEqual('0.013476505830589086655');
		expect(parser.evaluate('csch(-5)').text()).toEqual('-0.013476505830589086655');
		...
		expect(parser.evaluate('acosh(0)').text()).toEqual('1.5707963267948966193*i');
		...
		expect(parser.evaluate('asinh(1)').text()).toEqual('0.88137358701954302523');
		...
		expect(parser.evaluate('atanh(0.5)').text()).toEqual('0.5493061443340548457');
		expect(() => {
			parser.evaluate('atanh(1)');
		}).toThrow();
		expect(parser.evaluate('asech(5)').text()).toEqual('1.3694384060045658278*i');
		expect(parser.evaluate('asech(-5)').text()).toEqual('1.7721542475852274107*i');
		expect(() => {
			parser.evaluate('asech(0)');
		}).toThrow();
		expect(parser.evaluate('asech(0.5)').text()).toEqual('1.3169578969248167086');
		expect(parser.evaluate('asech(1)').text()).toEqual('0');

		expect(parser.evaluate('acsch(5)').text()).toEqual('0.19869011034924140647');
		expect(parser.evaluate('acsch(-5)').text()).toEqual('-0.19869011034924140648');
		expect(() => {
			parser.evaluate('acsch(0)');
		}).toThrow();
		expect(parser.evaluate('acsch(0.5)').text()).toEqual('1.4436354751788103425');
		expect(parser.evaluate('acsch(1)').text()).toEqual('0.88137358701954302523');
		expect(parser.evaluate('acoth(5)').text()).toEqual('0.20273255405408219099');
		expect(parser.evaluate('acoth(-5)').text()).toEqual('-0.20273255405408219099');
		expect(parser.evaluate('acoth(0)').text()).toEqual('1.5707963267948966193*i');
	...
	it('should return symbolic known values for hyperbolic functions at zero', () => {
		expect(parser.parse('cosh(0)').text()).toEqual('1');
		expect(parser.parse('sinh(0)').text()).toEqual('0');
		expect(parser.parse('tanh(0)').text()).toEqual('0');
	});

	it('should throw for hyperbolic functions at undefined points', () => {
		expect(() => {
			parser.parse('csch(0)');
		}).toThrow();
		expect(() => {
	...
	it('should handle atan with alternate representations of known values', () => {
		// 1/sqrt(3) and sqrt(3)/3 are the same value
		expect(parser.parse('atan(1/sqrt(3))').text()).toEqual('(1/6)*pi');
		expect(parser.parse('atan(-1/sqrt(3))').text()).toEqual('(-1/6)*pi');
	});
	...
	it('should calculate atan2 correctly', () => {
		...
		expect(parser.evaluate('atan2(-2, 123)').text()).toEqual('-0.016258729805129587692');
	});
});

describe('Trigonometric function regressions', () => {
	// Regression: https://github.com/jiggzson/nerdamer/issues/476
	it('preserves the negative sign for sine of negative pi multiples', () => {
		const negative = Number(Parser.evaluate('sin(-2*pi/180)').text());
		const positive = Number(Parser.evaluate('sin(2*pi/180)').text());

		expect(negative).toBeCloseTo(-positive, 18);
	});

	// Regression: https://github.com/jiggzson/nerdamer/issues/477
	it('preserves the negative sign for cosecant', () => {
		const negative = Number(Parser.evaluate('csc(-2)').text());
		const positive = Number(Parser.evaluate('csc(2)').text());

		expect(negative).toBeCloseTo(-positive, 18);
	});

	// Regression: https://github.com/jiggzson/nerdamer/issues/479
	it('does not equate pi+pi^2 with pi during trigonometric evaluation', () => {
		const result = Number(Parser.evaluate('sin(pi^2+pi)').text());
		const expected = Math.sin(Math.PI ** 2 + Math.PI);

		expect(result).toBeCloseTo(expected, 14);
	});

	// Regression: https://github.com/jiggzson/nerdamer/issues/398
	it('converts reciprocal trigonometric functions during evaluation', () => {
		const cases = [
			['1/sin(x)', 'csc(x)'],
			['1/cos(x)', 'sec(x)'],
			['1/tan(x)', 'cot(x)'],
		...
		// Regression: https://github.com/jiggzson/nerdamer/issues/398
		// This canonical conversion is deliberately limited to evaluation.
		expect(Expression.create('1/sin(x)').text()).toEqual('sin(x)^-1');

		for (const [input, expected] of cases) {
			expect(Expression.create(input).evaluate().text()).toEqual(expected);
		}
	});
});
