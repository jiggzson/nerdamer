...
describe('partfrac', () => {
	...
	describe('passthrough (no decomposition)', () => {
		it('preserves identity when decomposition reconstructs the input', () => {
			const input = Expression.create('(x+1)^-2');
			expect(partfrac(input, 'x')).toBe(input);
		});
...
describe('Partial-fraction regressions', () => {
	// Regression: https://github.com/jiggzson/nerdamer/issues/367
	it('evaluates partial fractions to an equivalent decomposition', () => {
		const source = Expression.create('(4*k-1)/(k*(k+2))');
		const result = Expression.create('partfrac((4*k-1)/(k*(k+2)),k)');

		expect(result.hasFunction('partfrac')).toBe(false);
		expect(result.minus(source).simplify().text()).toEqual('0');
	});
});
