...
describe('Expansion regressions', () => {
	// Regression: https://github.com/jiggzson/nerdamer/issues/3
	it('uses the accepted expansion path for opposite squared differences', () => {
		expect(Expression.create('(a-b)^2-(b-a)^2').expand().isZero()).toBe(true);
	});

	// Regression: https://github.com/jiggzson/nerdamer/issues/43
	it('expands a factored expression before subtracting a power', () => {
		expect(Expression.create('(x+5)*(x-3)-x^2').expand().eq(Expression.create('2*x-15'))).toBe(
			true
		);
	});

	// Regression: https://github.com/jiggzson/nerdamer/issues/272
	it('preserves variable exponents while expanding', () => {
		const source = nerdamer('((x^3+x)^x*(x^2+x)^x+1)*x');
		const expanded = source.expand();

		expect(expanded.evaluate({ x: 2 }).eq(source.evaluate({ x: 2 }))).toBe(true);
	});

	// Regression: https://github.com/jiggzson/nerdamer/issues/411
	it('expands multiplication by negative one correctly', () => {
		expect(nerdamer('expand(-1*(x-1))').text()).toEqual('1-x');
	});
});
