...
describe('Subst', () => {
	...
	it('substitutes powered simple variables without reparsing', () => {
		expect(Expression.create('3*x^2').subst('x', 'u').text()).toEqual('3*u^2');
		expect(Expression.create('x^-2').subst('x', 'u').text()).toEqual('u^-2');
	});

	it('does not reinterpret unrelated native values during simple substitution', () => {
		const variable = 'native_subst_roundtrip_probe';
		const expression = Expression.create(`x+${variable}`);
		const previous = Parser.KNOWN_VALUES[variable];

		try {
			Parser.KNOWN_VALUES[variable] = Expression.create(7);
			const result = expression.subst('x', 'u');

			expect(result.variables().sort()).toEqual([variable, 'u'].sort());
		} finally {
			if (previous) {
