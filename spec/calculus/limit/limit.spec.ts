...
describe('limit', () => {
	...
	describe('fallback behavior', () => {
		...
		it('round trips symbolic four-argument limits through the parser', () => {
			const unresolved = limit('sin(x)', 'x', 'Inf');
			const reparsed = Expression.create(unresolved.text());

			expect(reparsed.text()).toEqual(unresolved.text());
			expect(() => reparsed.simplify()).not.toThrow();
		});

		it('parses one-sided limit directions', () => {
			expect(Expression.create('limit(1/x,x,0,right)').text()).toEqual(Expression.Inf().text());
			expect(Expression.create('limit(1/x,x,0,left)').text()).toEqual(
				Expression.NegInf().text()
			);
		});
	});
...
describe('Limit regressions', () => {
	// Regression: https://github.com/jiggzson/nerdamer/issues/657
	it('computes same-degree rational limits at infinity correctly', () => {
		expect(
			nerdamer.limit('(2*x^2-2*x-2)/(x^2+4*x+3)', 'x', 'Infinity').text()
		).toEqual('2');
	});

	// Regression: https://github.com/jiggzson/nerdamer/issues/572
	it('preserves finite additive constants in limits at infinity', () => {
		expect(nerdamer.limit('e^(-x)+2', 'x', 'Infinity').text()).toEqual('2');
	});

	// Regression: https://github.com/jiggzson/nerdamer/issues/537
	it('uses the leading-coefficient ratio for rational limits at infinity', () => {
		expect(nerdamer.limit('(4*x^2-x)/(3*x^2+x)', 'x', 'Infinity').text()).toEqual('4/3');
		expect(nerdamer.limit('(7*x^3+4*x^2+x)/(12*x^3+6*x^2-2*x)', 'x', 'Infinity').text()).toEqual('7/12');
		expect(nerdamer.limit('(15*x^2+x)/(18*x^2-2*x)', 'x', 'Infinity').text()).toEqual('5/6');
	});
});