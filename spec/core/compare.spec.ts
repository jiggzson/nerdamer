...
describe('compare: equal', () => {
	it('should test for equality of expressions', () => {
		...
		// Inverse denominator signs
		expect(equal('x/(x-1)', '-x/(1-x)')).toBe(true);
		expect(equal('1/(x-1)+1/(1-x)', '0')).toBe(true);
		expect(equal('(x-1)/(1-x)', '-1')).toBe(true);
...
describe('compare: aggregate guardrails', () => {
...
describe('Comparison regressions', () => {
	// Regression: https://github.com/jiggzson/nerdamer/issues/133
	it('returns expression values for true and false comparisons', () => {
		expect(nerdamer('1<2').text()).toEqual('1');
		expect(nerdamer('2<1').text()).toEqual('0');
		expect(nerdamer('1<2').evaluate().text()).toEqual('1');
		expect(nerdamer('2<1').evaluate().text()).toEqual('0');
	});

	// Regression: https://github.com/jiggzson/nerdamer/issues/303
	it('keeps unequal quadratic expressions distinct', () => {
		const left = nerdamer('(k-1)^2+(k-1)^2');
		const right = nerdamer('(2*k-2)^2');

		expect(left.eq(right)).toBe(false);
		expect(left.evaluate({ k: 2 }).text()).toEqual('2');
		expect(right.evaluate({ k: 2 }).text()).toEqual('4');
	});

	// Regression: https://github.com/jiggzson/nerdamer/issues/543
	it('recognizes algebraically equal expanded expressions', () => {
		expect(Expression.create('4*(x+1)*(x+4)').eq(Expression.create('4*x^2+20*x+16'))).toBe(true);
		expect(Expression.create('(x+1)*(x-1)').eq(Expression.create('x^2-1'))).toBe(true);
	});

	// Regression: https://github.com/jiggzson/nerdamer/issues/602
	it('distinguishes an identity from an unconstrained variable equality', () => {
		expect(Expression.create('a*b').eq('b*a')).toBe(true);
		expect(Expression.create('a').eq('b')).toBe(false);
	});
});
