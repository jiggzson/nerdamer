...
function defaultTolFromPrecision(): Decimal {
	...
	const p = Decimal.precision ?? DEFAULT_DECIMAL_PRECISION;
...
export function definiteIntegrate(
	...
	if (allowInfinite && (isInfA || isInfB)) {
		if (!isInfA && isInfB && B.isPos()) {
			...
			const eps = new Decimal(10).pow(-(Decimal.precision ?? DEFAULT_DECIMAL_PRECISION) + INFINITE_BOUND_PRECISION_SLACK);
