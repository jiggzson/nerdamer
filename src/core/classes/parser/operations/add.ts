		...
		if (a.isNUM() && b.isNUM()) {
			...
			if (a.isInf()) {
				...
				if ((x.isNegative() && !y.isNegative()) || (y.isNegative() && !x.isNegative())) {
					throw new UndefinedError(message('infinityMinusInfinityUndefined'));
				}
			}
...
function canJoin(a: Expression, b: Expression) {
	// In order for SUM or GRP to be able to join, it needs to have a multiplier and a power of one
	return (
		// Any value can be added to SUM
		(a.type === SUM ||
		...
		a.isLinear() &&
		(!a.multiplier || (a.multiplier && a.getMultiplier().isOne()))
	);
}
...
export function join(a: Expression, b: Expression, isGroup = false): Expression {
	...
	const expressionType = isGroup ? GRP : SUM;
	...
	// Case 3
		...
		const retvalExpressions = retval.getElements();
		...
		elements = Object.values(retvalExpressions);
	}

	// Some cleanup. The expression may have now become a single element. We'll know this because
	// the sub-elements will only be one element long. If that's the case then that become the reval
	if ((retval.isSum() || retval.isProduct()) && elements.length === 1) {
