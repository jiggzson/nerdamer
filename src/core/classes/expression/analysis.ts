import { PolynomialError, message } from '../../errors';
...
export function coeffs(x: Expression, variables: string[], coeffsObj?: CoeffObject) {
	...
	if (x.isComplex() && !variables.includes(Expression.imaginary)) {
		...
		imaginaryCoeffs.each((coeff, power) =>
			cObj.add(power, coeff.times(Expression.Img()))
		);
	...
	if (x.isProduct()) {
		let coefficient = one();
		let product = Expression.fromRational(x.getMultiplier());
