...
export class Pattern extends BaseConverter {
	...
	private args: string[] = [];
	private c: string;
	private constCount = 0;
	private expression: Expression;

	private multiplier = '';
	private p: string;
	private powerCount = 0;
	private referenceMap: { [label: string]: string } = {};

	private reuseReferences: boolean;
	private v: string;
	private variables: { [variable: string]: string } = {};
	patternString = '';
	...
	constructor(
		...
		this.v = resolvedOptions.variableSymbol;
		this.p = resolvedOptions.powerSymbol;
		this.c = resolvedOptions.constantSymbol;
		this.reuseReferences = resolvedOptions.reuseReferences;

		this.initializeVariables(variables);
		this.patternString = this.convert(this.expression, resolvedOptions);
	}
