...
const MAX_DISTINCT_DEGREE_ITERATIONS = 64n;
const BASE_PRIME_BUDGET = 32;
const PRIME_BUDGET_DEGREE_DIVISOR = 4;
const PRIME_BUDGET_BIT_DIVISOR = 64;
const MAX_PRIME_BUDGET = 256;
const DEFAULT_MIN_FACTOR_PRIME = 3;
...
function berlekampFactorFp(fIn: ZPoly, p: bigint): ZPoly[] {
	...
	for (let iter = 1n; iter <= MAX_DISTINCT_DEGREE_ITERATIONS; iter++) {
		let changed = false;
		const newFactors: ZPoly[] = [];
		for (const fac of factors) {
			if (degZPoly(fac) <= 1) {
...
function primeBudgetForUniZ(f: ZPoly): number {
	...
	// Scale the baseline prime budget with polynomial degree and coefficient size.
	let budget = BASE_PRIME_BUDGET +
		Math.ceil(n / PRIME_BUDGET_DEGREE_DIVISOR) +
		Math.ceil(bits / PRIME_BUDGET_BIT_DIVISOR);
	// Clamp to a reasonable range.
	if (budget < BASE_PRIME_BUDGET) {
		budget = BASE_PRIME_BUDGET;
	}
	if (budget > MAX_PRIME_BUDGET) {
...
function findGoodPrimeBig(
	f: ZPoly,
	minPrime = DEFAULT_MIN_FACTOR_PRIME,
...
export function factorPolynomialBig(input: ZPoly): FactorizationResultZ {
	...
	const sqfParts: Array<[ZPoly, number]> = sqf.length === 0 ? [[f, 1]] : sqf;
