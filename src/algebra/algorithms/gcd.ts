...
const DEFAULT_ZIPPEL_MAX_TRIES = 32;
const CANDIDATE_VERIFICATION_SAMPLES = 6;
const CANDIDATE_VERIFICATION_SEED_BASE = 900000;
const FALLBACK_SPECIALIZATION_ATTEMPTS = 8;
const MAX_SCHEDULED_GRID_SAMPLES = 64;
const FALLBACK_ATTEMPT_SEED_STRIDE = 1_000_000;
const FALLBACK_ROUND_SEED_STRIDE = 100_000;
const SUPERFLUOUS_VARIABLE_EVALUATION_PRIMES = [
	17n, 19n, 23n, 29n, 31n, 37n, 41n, 43n, 47n, 53n,
];
const INTERPOLATION_SAMPLE_PRIMES = [
	2n, 3n, 5n, 7n, 11n, 13n, 17n, 19n, 23n, 29n, 31n, 37n, 41n, 43n, 47n,
];
const MIN_INTERPOLATION_POINTS = 3;
const MAX_INTERPOLATION_POINTS = 15;
const RANDOM_64_BIT_MASK = (1n << 64n) - 1n;
const RANDOM_32_BIT_SHIFT = 32n;
const RANDOM_32_BIT_MASK = 0xffffffffn;
const RANDOM_LCG_MULTIPLIER = 6364136223846793005n;
const RANDOM_LCG_INCREMENT = 1442695040888963407n;
const RANDOM_HASH_OFFSET = 1469598103934665603n;
const RANDOM_HASH_PRIME = 1099511628211n;
const RANDOM_HASH_SEED_XOR = 0x9e3779b97f4a7c15n;
const RANDOM_EVAL_MIN_VALUE = 2n;
const RANDOM_EVAL_VALUE_RANGE = 96n;
...
interface ZippelGCDOptions {
	...
	maxTries?: number; // defaults to DEFAULT_ZIPPEL_MAX_TRIES
...
function zippelGCD(f: MultiPoly, g: MultiPoly, vars: Vars, opts: ZippelGCDOptions = {}): MultiPoly {
	...
	// We substitute them with primes, then drop those variables and recurse.
	{
		...
		if (superfluousVars.length > 0) {
			...
			for (let i = 0; i < superfluousVars.length; i++) {
				...
				const prime =
					SUPERFLUOUS_VARIABLE_EVALUATION_PRIMES[
						i % SUPERFLUOUS_VARIABLE_EVALUATION_PRIMES.length
					];
	...
	const maxTries = opts.maxTries ?? DEFAULT_ZIPPEL_MAX_TRIES;
	...
	function verifyCandidate(Hcand: MultiPoly): boolean {
		for (let k = 0; k < CANDIDATE_VERIFICATION_SAMPLES; k++) {
			const values = randomEvalPoint(vars, mainVar, CANDIDATE_VERIFICATION_SEED_BASE + k);
			const fu = evalToUnivariate(fPrimMP, vars, mainVar, values);
			const gu = evalToUnivariate(gPrimMP, vars, mainVar, values);
			if (uniIsZeroBig(fu) || uniIsZeroBig(gu)) {
				continue;
	...
	for (let attempt = 0; attempt < FALLBACK_SPECIALIZATION_ATTEMPTS; attempt++) {
		const coeffPolys: MultiPoly[] = new Array(hDeg + 1);
		for (let d = 0; d <= hDeg; d++) {
			coeffPolys[d] = MultiPoly.zero();
		}

		// Each sampled point produces the complete univariate GCD. Reuse that result for
		// every coefficient instead of recomputing the same GCD hDeg + 1 times.
		const gcdSampleSets = new Map<number, GcdSample[]>();
		for (let d = 0; d <= hDeg; d++) {
			let polyDmv: MultiPoly | null = null;
			for (let r = 0; r < 4 && !polyDmv; r++) {
				let gcdSamples = gcdSampleSets.get(r);
				if (!gcdSamples) {
					gcdSamples = sampleGcdAcrossPoints(
						F.primitive,
						G.primitive,
						vars,
						mainVar,
						Math.min(maxTries * (r + 1), MAX_SCHEDULED_GRID_SAMPLES),
						attempt * FALLBACK_ATTEMPT_SEED_STRIDE + r * FALLBACK_ROUND_SEED_STRIDE,
						opts.gridMap
					);
					gcdSampleSets.set(r, gcdSamples);
				}

				const samples = gcdSamples.map(sample => ({
					values: sample.values,
					y: d < sample.coefficients.length ? sample.coefficients[d] : 0n,
				}));
				try {
...
// Collect full univariate GCD samples once so every reconstructed coefficient can reuse them.
type GcdSample = { values: Record<string, bigint>; coefficients: bigint[] };

function sampleGcdAcrossPoints(
	f: MultiPoly,
	g: MultiPoly,
	vars: Vars,
	mainVar: string,
	...
	function pointsForVar(v: string): bigint[] {
		...
		if (otherVars.length && v === otherVars[0]) {
			...
			const n = Math.min(
				Math.max(bound + 1, MIN_INTERPOLATION_POINTS),
				MAX_INTERPOLATION_POINTS
			); // cap to avoid combinatorial blowup
			return INTERPOLATION_SAMPLE_PRIMES.slice(0, n);
		}
		return INTERPOLATION_SAMPLE_PRIMES.slice(0, MIN_INTERPOLATION_POINTS);
	}
	...
	function rec(i: number, budget: { n: number }): void {
		...
		if (i === otherVars.length) {
			...
			out.push({ values, coefficients: huFull });
			budget.n--;
			return;
		}
		const v = otherVars[i];
	...
	for (let t = 0; out.length < Math.min(6, maxTries) && t < maxTries * 2; t++) {
		...
		out.push({ values, coefficients: hu });
	}

	if (out.length === 0) {
		return [{ values: randomEvalPoint(vars, mainVar, 9999), coefficients: [] }];
	}
	return out;
}
...
function randomEvalPoint(vars: Vars, mainVar: string, seed: number): Record<string, bigint> {
	...
	let state = BigInt(seed) ^ RANDOM_HASH_SEED_XOR;
	function nextU32(): bigint {
		state = (state * RANDOM_LCG_MULTIPLIER + RANDOM_LCG_INCREMENT) & RANDOM_64_BIT_MASK;
		return (state >> RANDOM_32_BIT_SHIFT) & RANDOM_32_BIT_MASK;
	}
	function hashVar(s: string): bigint {
		let h = RANDOM_HASH_OFFSET;
		for (let i = 0; i < s.length; i++) {
			h ^= BigInt(s.charCodeAt(i));
			h *= RANDOM_HASH_PRIME;
			h &= RANDOM_64_BIT_MASK;
		}
		return h;
	}
	for (const v of vars) {
		...
		state ^= hashVar(v);
		const r = nextU32();
		values[v] = RANDOM_EVAL_MIN_VALUE + (r % RANDOM_EVAL_VALUE_RANGE);
	}
	return values;
}
