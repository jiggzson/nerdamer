...
export function reduceTrigPower(e: Expression): Expression {
	...
	if (nBig < 0n) {
		const positive = reduceTrigPower(
			isSin
				? sin(u).pow(Expression.Number(nAbs))
				: cos(u).pow(Expression.Number(nAbs))
		);
...
//          → 1 - 2·sin²(u)
//
// Reverse direction (useful for reducing products):
 ...
 */
export function convertToDoubleAngle(x: Expression): Expression {
	if (!x.isProduct()) {
		return x;
	}
