 ...
 */
export function partfrac(x: ExpressionInput, variable?: ExpressionInput): Expression {
	...
	if (v && vars.length > 0) {
		...
		if (den.hasVariable(v)) {
			...
			if (Expression.create(remainder).isZero()) {
				...
				const factors = polyFactorExpressions(den);
...
function extractNumDen(expr: Expression, _v: string): { num: Expression; den: Expression } {
	...
	if (expr.isProduct()) {
		...
		num = num.times(Expression.fromRational(expr.getMultiplier()));
	...
	if (expr.getPower().sign() === -1) {
		return { num: Expression.fromRational(expr.getMultiplier()), den: expr.invert() };
	}
 ...
 */
function collectFactors(factors: Expression[], v: string): DenominatorFactor[] {
...
function decompose(
	...
	while (evalPoints.length < totalUnknowns) {
		const candidate = Expression.Number(pointVal);
	...
	const cofactors = terms.map(term =>
		computeCofactor(denominator, term.factor, term.power)
	);
...
function computeCofactor(
	...
	// Q / factor^power
	const facPow = power === 1 ? fac : fac.pow(Expression.Number(power));
