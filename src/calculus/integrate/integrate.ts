...
import { ASIN, ASINH, ATAN, COS, INTEGRATE, LOG, SIN } from '../../core/classes/parser/constants';
...
export function integrate(
	...
	if (depth++ > MAX_DEPTH) {
		...
		if (!expr.hasVariable(dx)) {
			...
			let sum = zero();
			expr.each((e: Expression) => {
				sum = sum.plus(integrate(e, dx, depth));
			});
			retval = sum;
		} else {
			retval = tableOfIntegrals.lookup(new Pattern(expr, [dx]), depth);
		}
...
function tryIntegratePolynomial(expr: Expression, dx: string): Expression | undefined {
	if (!expr.isPolynomialLike() || hasTranscendental(expr, dx)) {
		return undefined;
	}

	let polynomial: Polynomial;
	try {
		polynomial = new Polynomial(expr, [dx]);
	} catch {
		return undefined;
	}
...
function tryIntegrateLogPower(
	expr: Expression,
	dx: string,
	depth: number
): Expression | undefined {
	if (!expr.isFunction(LOG)) {
		return undefined;
	}

	const power = expr.getPower();
	if (!power.isInteger() || !power.gt(one())) {
		return undefined;
	}
	...
	let argumentPolynomial: Polynomial;
	try {
		argumentPolynomial = new Polynomial(args[0], [dx]);
	} catch {
		return undefined;
	}
	...
	const linearCoefficient = coeffs[1] ?? zero();
	if (linearCoefficient.isZero()) {
		return undefined;
	}

	const lowerPower = expr.getBase().pow(power.minus(one()));
	const lowerIntegral = integrate(lowerPower, dx, depth);
	if (lowerIntegral.isFunction(INTEGRATE)) {
		return undefined;
	}
...
function tryIntegrateRational(expr: Expression, dx: string, depth: number): Expression | undefined {
	...
	if (degNum >= degDen) {
		try {
			const [quotient, rem] = divide(num, den, undefined, [dx]);
...
function tryIntegratePowerQuotient(
	...
	try {
		[quotient, remainder] = divide(num, denBase, undefined, [dx]);
	} catch {
		return undefined;
	}
