...
import { minusTwo, one, two, zero } from '../../core/classes/expression/shortcuts';
import { ABS, ATAN, COS, LIMIT, LOG, SGN, SIN, SQRT } from '../../core/classes/parser/constants';
...
/** Direction from which the independent variable approaches a finite point. */
...
function tryRewriteIndeterminate(
	...
	if (expr.getDenominator().isOne() && expr.isProduct()) {
		...
		if (zeroIdx !== -1 && infIdx !== -1) {
			...
			let rewritten = zeroFactor.div(one().div(infFactor));
...
function tryComposition(
	...
	if (expr.isFunction() && !expr.isFunction(LIMIT)) {
		...
		if (innerLimit.isNegInf()) {
			switch (name) {
				case ATAN:
					return Expression.Pi().div(minusTwo());
...
export function limit(
	...
	if (val.isPosInf() || val.isNegInf()) {
		...
		if (expr.isFunction(SGN) || expr.isFunction(ABS)) {
	...
	if (!retval) {
		retval = tableOfLimits.lookup(new Pattern(expr, [x.text()]), 0);
	}

	retval ??= Expression.toFunction(LIMIT, [expr, x, val, dir]);
	if (asDecimal) {
